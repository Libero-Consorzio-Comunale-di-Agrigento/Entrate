...
class DatiCatastaliUgualiViewModel extends AnomalieDatiCatastaliViewModel {
    ...
    init(@ContextParam(ContextType.COMPONENT) Window w
        ...
        if (ricercaInArchivio) {
            def ricercaArchivio = gestioneAnomalieService.getOggettiDaAchivioEstremiUguali(oggettoDTORicerca)
            oggettiDaArchivio = ricercaArchivio.lista
            listaFiltri = [ricercaArchivio.filtro]

            def ricercaCatasto = gestioneAnomalieService.getOggettiDaCatastoEstremiUguali(oggettoDTORicerca)
