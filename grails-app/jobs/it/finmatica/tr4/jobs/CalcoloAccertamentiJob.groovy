...
class CalcoloAccertamentiJob {

    private static Log log = LogFactory.getLog(CalcoloAccertamentiJob)
    ...
    def group = "CalcoloAccertamenti"
    def description = "Calcolo accertamenti"
    ...
    private def executeWithoutListaContribuenti(JobExecutionContext context) {
        log.info 'Calcolo accertamenti senza lista contribuenti'
        ...
        try {
            imposteService.calcoloAccertamenti(parametriCalcolo, user)
    ...
    private def executeWithListaContribuenti(JobExecutionContext context, def listaContribuenti) {
        ...
        log.info 'Calcolo accertamenti da lista contribuenti'
        ...
        listaContribuenti.each {
            ...
            try {
                ...
                def idPratica = imposteService.calcoloAccertamenti(parametriCalcolo, user)

                if (idPratica != null) {
                    numContribuentiTrattati++
                    PraticaTributo pratica = PraticaTributo.get(idPratica)
                    def logString = "Generato accertamento per ${contribuente.soggetto.nome} ${contribuente.soggetto.cognome} codice fiscale ${contribuente.codFiscale} anno ${parametriCalcolo.anno} importo ${commonService.formattaValuta(pratica.importoTotale)}\n"
                    logFile.append(logString)

                } else {
                    numContribuentiInErrore++
                    logFile.append("Non generato accertamento per ${contribuente.soggetto.nome} ${contribuente.soggetto.cognome} codice fiscale ${contribuente.codFiscale} anno ${parametriCalcolo.anno}\n")
                }
            } catch (Exception e) {
                numContribuentiInErrore++
                e.printStackTrace()
                logFile.append("Errore in generazione accertamento per ${contribuente.soggetto.nome} ${contribuente.soggetto.cognome} codice fiscale ${contribuente.codFiscale} anno ${parametriCalcolo.anno}: " + e.getMessage() + "\n")
            }
        }
