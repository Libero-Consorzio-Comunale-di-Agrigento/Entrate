...
--changeset abrandolini:20250326_152438_f_valore stripComments:false runOnChange:true 
 
create or replace function F_VALORE
(a_valore            IN number
...
w_return      number;
BEGIN
   if a_tipo_ogge = 2 or a_tipo_ogge = 4 then
      RETURN a_valore;
   end if;
   aliquota_new := 0;
   BEGIN
   ...
   EXCEPTION
      ...
      aliquota_dic := 0;
   END;
   if nvl(a_flag_valore_rivalutato,'N') = 'N' and a_tipo_pratica = 'A' then
      aliquota_dic := 0;
   end if;
