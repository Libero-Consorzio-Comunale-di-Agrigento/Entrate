...
--changeset abrandolini:20250326_152438_f_dovuto stripComments:false runOnChange:true 
 
create or replace function F_DOVUTO
(p_ni             number
,p_anno           number
,p_titr           varchar2
...
/*************************************************************************
 NOME:        F_DOVUTO
 DESCRIZIONE: Dati ni, anno e tipo tributo e anno da cui considerare le
              dichiarazioni, ritorna il dovuto a seconda del parametro
              p_richiesta
              Il parametro p_tributo vale -1 per trattare tutti i tributi.
 RITORNA:     number              Importo dovuto dal contribuente.
 NOTE:        P_richiesta puo' assumere i seguenti valori:
              D  - Dovuto annuo (al netto degli sgravi)
              DR - Dovuto arrotondato x oggetto (al netto degli sgravi)
              DL - Dovuto liquidato
              S - Sgravio
              ATTENZIONE: per la TARSU il dovuto e' sempre arrotondato
              (ma gli sgravi no).
 Rev.    Date         Author      Note
 001     19/10/2018   VD          Aggiunto parametro cod_fiscale
                                  facoltativo. Se nullo, si ricava il
                                  codice fiscale dall'ni.
 000     28/02/2014   XX          Prima emissione.
******************************************************************************/
  w_dovuto        number;
  w_sgravi        number;
  w_ruolo         number;
...
begin
  ...
  -- per migliorare il piano d'accesso delle select successive
  -- (VD - 19/10/2018): se il codice fiscale passato non e' nullo,
  --                    si utilizza tale parametro nelle ricerche
  if p_cod_fiscale is null then
