...
--changeset abrandolini:20250326_152438_f_importo_da_scalare_sup2s stripComments:false runOnChange:true 
 
create or replace function F_IMPORTO_DA_SCALARE_SUP2S
/*************************************************************************
 NOME:        F_IMPORTO_DA_SCALARE_SUP2S
 DESCRIZIONE: Bovezzo
              La funzione viene utilizzata per i suppletivi dei ruoli semestrali
              funziona correttamente solo se il suppletivo è successivo al ruolo
              del secondo semestre e solo per il primo supplettivo per ogpr
              gli sgravi non sono stati provati.
 ...
 001     06/11/2018   VD          Selezione importi calcolati con tariffa
                                  base.
 000     19/05/2010   XX          Prima emissione.
*************************************************************************/
(p_ruolo                          number
,p_cf                             varchar2
,p_anno                           number
...
if w_tiev = 'C' then
   ...
   BEGIN
      select sum(decode(ruol.importo_lordo,'S',ruco.importo - nvl(ogim.addizionale_eca,0)
                  ...
                  * 12 / decode(nvl(sum(ruco.mesi_ruolo),12),0,12,sum(ruco.mesi_ruolo))
       ...
       where ruol.ruolo            = ruco.ruolo
         ...
         and p_dal                <=
             nvl(ogva.al,to_date('3112'||lpad(to_char(p_anno),4,'0'),'ddmmyyyy')) + 1
...
END;
/* End Function: F_IMPORTO_DA_SCALARE_SUP2S */
/
