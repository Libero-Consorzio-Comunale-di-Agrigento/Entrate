...
--changeset abrandolini:20250326_152438_f_importo_f24_viol_tefa stripComments:false runOnChange:true 
 
create or replace function F_IMPORTO_F24_VIOL_TEFA
(a_importo              in number
...
/**
   ...
   000   22/09/2022  XX      Prima emissione
**/
is
  --
  nImporto            number;
...
BEGIN
  ...
  --
   if nvl(a_tipo_causale,'*') = 'E' and nvl(a_flag_magg_tares,'N') = 'N'
      and a_tipo_tributo in ('TEFA','TARSU','LORDO') then
      BEGIN
      ...
      if a_tipo_tributo = 'TARSU' then
         ...
         if a_tipo_tributo = 'TEFA' then
            nImporto := round(w_importo * nAdd_Pro_perc / 100,2);
         else
   ...
   Return nImporto;
END;
/* End Function: F_IMPORTO_F24_VIOL_TEFA */
/
