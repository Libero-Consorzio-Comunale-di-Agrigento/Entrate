...
--changeset abrandolini:20250326_152429_stampa_solleciti_trmi stripComments:false runOnChange:true 
 
create or replace package STAMPA_SOLLECITI_TRMI is
/******************************************************************************
 NOME:        STAMPA_SOLLECITI_TRMI
 DESCRIZIONE: Funzione per stampa avvisi di sollecito tributi minori
              TributiWeb
 ANNOTAZIONI: -
 REVISIONI:
 Rev.  Data        Autore  Descrizione
 ----  ----------  ------  ----------------------------------------------------
 002   22/02/2023  RV      #Issue61562
                           Aggiunto data_scadenza
 001   01/02/2023  RV      #Issue61562
                           Aggiunto totali evasa e sanzioni + interessi
 000   24/01/2023  RV      #Issue61562
                           Prima emissione
******************************************************************************/
  function principale
...
end STAMPA_SOLLECITI_TRMI;
/
create or replace package body STAMPA_SOLLECITI_TRMI is
/******************************************************************************
 NOME:        STAMPA_SOLLECITI_TRMI
 DESCRIZIONE: Funzione per stampa avvisi di sollecito tributi minori TributiWeb
 ANNOTAZIONI: -
 REVISIONI:
 Rev.  Data        Autore  Descrizione
 ----  ----------  ------  ----------------------------------------------------
...
----------------------------------------------------------------------------------
  ...
  ******************************************************************************/
    rc                                  sys_refcursor;
    v_rc                                sys_refcursor;
    v_imposta_evasa_tot                 varchar2(20);
    v_sanz_int_importo_tot              varchar2(20);
    type t_imposta_evasa is record
    ( descrizione                       varchar2(100),
      importo_sanz                      varchar2(20),
      perc_sanzione                     varchar2(20),
      perc_riduzione                    varchar2(20),
      importo_tot                       number(10, 2),
      st_importo_tot                    varchar2(20)
    );
    v_imposta_evasa t_imposta_evasa;
    type t_sanz_int is record
    ( cod_sanzione                      number(4),
      ...
      st_importo                        varchar2(20),
      cod_tributo_f24                   varchar2(4),
      descr_codice_f24                  varchar2(100),
      lbl_irrogazioni_sanz_int          varchar2(100),
      importo_tot                       number(10, 2),
      st_importo_tot                    varchar2(20)
    );
    v_sanz_int t_sanz_int;
  begin
    v_imposta_evasa_tot := '';
    v_rc := imposta_evasa(a_pratica, a_modello);
    loop
      fetch v_rc
        into v_imposta_evasa;
      exit when v_rc%notfound;
      v_imposta_evasa_tot := v_imposta_evasa.st_importo_tot;
      exit;
    end loop;
    v_sanz_int_importo_tot := '';
    v_rc := sanzioni_interessi(a_pratica, a_modello);
    loop
      fetch v_rc
        into v_sanz_int;
      exit when v_rc%notfound;
      v_sanz_int_importo_tot := v_sanz_int.st_importo_tot;
      exit;
    end loop;
    open rc for
      select a_modello modello
           ...
           , to_char(prtr_acc.data_scadenza,'dd/mm/yyyy') data_scadenza
           , prtr_acc.numero
           ...
           , f_descrizione_timp(a_modello,'TOT_SANZ_INT') l_sanz_int_importo_tot
           , v_sanz_int_importo_tot as st_sanz_int_importo_tot
           , f_descrizione_timp(a_modello,'TOT_EVASA') l_imposta_evasa_tot
           , v_imposta_evasa_tot as st_imposta_evasa_tot
  ...
  begin
    open rc for
      select sanzioni.descrizione descrizione
           ...
           , sum(sapr.importo) over() as importo_tot
           , stampa_common.f_formatta_numero(sum(sapr.importo) over(),'I','S') as st_importo_tot
