...
--changeset abrandolini:20250326_152423_calcolo_sanzioni_tosap stripComments:false runOnChange:true 
 
create or replace procedure CALCOLO_SANZIONI_TOSAP
...
IS
C_TIPO_TRIBUTO     CONSTANT varchar2(5) := 'TOSAP';
...
BEGIN   -- CALCOLO_SANZIONI_TOSAP
  ...
  IF a_anno = a_anno_denuncia THEN
   IF a_data_concessione is not NUll THEN
       IF to_number(a_data_denuncia - a_data_concessione) > 60 THEN
            w_cod_sanzione := C_TARD_DEN_SUP_30;
            inserimento_sanzione(w_cod_sanzione,C_TIPO_TRIBUTO,a_pratica,a_oggetto_pratica,w_maggiore_imposta,NULL,a_utente);
            w_cod_sanzione := C_TARD_DEN_SUP_30 + C_NUOVO;
            inserimento_sanzione(w_cod_sanzione,C_TIPO_TRIBUTO,a_pratica,a_oggetto_pratica,w_maggiore_imposta,NULL,a_utente);
       ELSIF to_number(a_data_denuncia - a_data_concessione) > 30 THEN
  ...
  if a_gestione_versamenti = 'S' then
     inserimento_sanzione_vers(a_cod_fiscale,a_anno,a_pratica, a_oggetto_pratica, a_imposta,
                               a_imposta_dichiarata, a_anno_denuncia, a_data_concessione, C_TIPO_TRIBUTO, a_utente);
  end if;
     ...
     if w_tot_sanzioni > 0 then
        BEGIN
           select sanzione
             into w_imp_sanzione
             from sanzioni
            where cod_sanzione = 97
              and tipo_tributo = C_TIPO_TRIBUTO
           ;
           if nvl(w_imp_sanzione,0) > 0 then
              w_cod_sanzione := 97;
              inserimento_sanzione(w_cod_sanzione,C_TIPO_TRIBUTO,a_pratica,a_oggetto_pratica,NULL,w_imp_sanzione,a_utente);
           end if;
        EXCEPTION
...
END;
/* End Procedure: CALCOLO_SANZIONI_TOSAP */
/
