...
--changeset abrandolini:20250326_152423_liquidazioni_ici_rendita stripComments:false runOnChange:true 
 
create or replace procedure LIQUIDAZIONI_ICI_RENDITA
/*************************************************************************
 Rev. Data          Autore    Descrizione
 8    25/09/2023    RV        #66351 - Accetta valore_dic null per tipi 1 e tipi 3 con Ctg. E ed F
 7    02/08/2023    VM        #65986 - Fix calcolo mesi esclusione in base al flag esclusione
 6    08/05/2023    AB        Controllo periodi considerando data_fine >= w_anno
 5    31/01/2023    AB        Controllo date utilizzando anche periodi_riog
 4    21/12/2022    AB        Nel caso di importo a 0 e mesi esclusione
 ...
 3    15/12/2022    AB        Gestione del da_mese_possesso per il
                              controllo dei riog
 2    02/08/2022    VD        Aggiunto test su flag esclusione per non
 ...
 0    01/12/2008    XX        Prima emissione.
*************************************************************************/
(a_anno                           number,
 a_pratica                        number,
...
CURSOR sel_ogim (p_anno number, p_cod_fiscale varchar2) IS
   ...
   -- Altri Fabbricati --
    ...
    ;
BEGIN -- LIQUIDAZIONI_ICI_RENDITA
   wf := 0;
   ...
   FOR rec_ogim IN sel_ogim (a_anno, a_cod_fiscale) LOOP
      ...
      ELSE
         ...
         IF rec_ogim.tipo_pratica = 'A' and rec_ogim.anno = a_anno THEN
            ...
            a_imp_dovuta_dic_ab             := a_imp_dovuta_dic_ab - nvl(rec_ogim.imp_dovuta_ab,0);
            a_imp_dovuta_acconto_dic_ab     := a_imp_dovuta_acconto_dic_ab - nvl(rec_ogim.imp_dovuta_acconto_ab,0);
            ...
            a_imp_dovuta_dic_ter            := a_imp_dovuta_dic_ter - nvl(rec_ogim.imp_dovuta_ter,0);
            a_imp_dovuta_acconto_dic_ter    := a_imp_dovuta_acconto_dic_ter - nvl(rec_ogim.imp_dovuta_acconto_ter,0);
            ...
            a_imp_dovuta_dic_aree           := a_imp_dovuta_dic_aree - nvl(rec_ogim.imp_dovuta_aree,0);
            a_imp_dovuta_acconto_dic_aree   := a_imp_dovuta_acconto_dic_aree - nvl(rec_ogim.imp_dovuta_acconto_aree,0);
            ...
            a_imp_dovuta_dic_altri          := a_imp_dovuta_dic_altri - nvl(rec_ogim.imp_dovuta_altri,0);
            a_imp_dovuta_acconto_dic_altri  := a_imp_dovuta_acconto_dic_altri - nvl(rec_ogim.imp_dovuta_acconto_altri,0);
...
--                  and riog.oggetto = rec_ogim.oggetto
           ...
           --dbms_output.put_line('Categoria/Classe '||rec_ogim.categoria_catasto||'/'||rec_ogim.classe_catasto);
            if  w_num_riog = 1
             and w_min_dal_riog <= to_date('01'||lpad(rec_ogim.da_mese_possesso,2,'0')||lpad(to_char(w_anno),4,'0'),'ddmmyyyy')
--             and w_max_al_riog  >= to_date('3112'||lpad(to_char(w_anno),4,'0'),'ddmmyyyy')
             and w_max_al_riog  >= last_day(
                                     to_date('01'||lpad(rec_ogim.da_mese_possesso+rec_ogim.mesi_possesso-1,2,'0')||
...
--               if rec_ogim.oggetto = 10693 then
--                RAISE_APPLICATION_ERROR (-20999,'w_num_riog '||w_num_riog||' w_valore_ogpr '||w_valore_ogpr );
--               end if;
            BEGIN
              ...
              THEN
               ...
               IF nvl(w_imponibile_10_acconto,0) > 0 THEN  -- INIZIO CONTROLLO ANOMALIE 10 , 110
                  ...
                  ELSIF w_check = 1 THEN
                    aggiornamento_sanzione_liq(w_cod_sanzione,C_TIPO_TRIBUTO
...
EXCEPTION
  ...
  WHEN others THEN
       ROLLBACK;
       RAISE_APPLICATION_ERROR (-20999,'Errore in LIQUIDAZIONI ICI RENDITA -'||to_char(wf)||'- ('||SQLERRM||')');
END;
/* End Procedure: LIQUIDAZIONI_ICI_RENDITA */
/
