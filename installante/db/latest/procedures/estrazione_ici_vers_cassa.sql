...
--changeset abrandolini:20250326_152423_estrazione_ici_vers_cassa stripComments:false runOnChange:true 
 
create or replace procedure ESTRAZIONE_ICI_VERS_CASSA
(a_anno               in number
,a_fonte              in number
...
select vers.data_pagamento
     ...
     , vers.pratica
     , vers.anno                                                                vers_anno
     , to_number(to_char(vers.data_pagamento,'yyyy'))                           anno_pagamento
...
select vers.data_pagamento
 ...
 where vers.tipo_tributo             = 'ICI'
   ...
   and fonte                         = p_fonte
   and vers.data_pagamento    between a_dal
                                  and p_al
 order by
       vers.cod_fiscale
;
...
BEGIN
   ...
   -----------------------------------------------------------------------------
   FOR rec_vers in sel_vers(a_dal,a_al,a_fonte)
   LOOP
      w_numero := w_numero + 1;
      ...
      begin
         ...
         values( lpad(to_char(w_numero),12,'0')
               ...
               ||substr(to_char(rec_vers.vers_anno),3,2)                        -- Anno imposta
               ...
               ||to_char(rec_vers.vers_anno)                                    -- Periodo di riferimento del versamento
   ...
   -----------------------------------------------------------------------------
   FOR rec_viol in sel_viol(a_dal,a_al,a_fonte)
   LOOP
      w_numero := w_numero + 1;
      w_progr_record := w_progr_record + 1;
      begin
...
END;
/* End Procedure: ESTRAZIONE_ICI_VERS_CASSA */
/
