...
--changeset abrandolini:20250326_152423_liquidazioni_ici_riog stripComments:false runOnChange:true 
 
create or replace procedure LIQUIDAZIONI_ICI_RIOG
/*************************************************************************
 ...
 3    15/12/2022    AB        Gestione del da_mese_possesso per il
                              controllo dei riog e tolto il controllo
                              della rev 2
...
-- a_data_scad_acconto    IN date, -- non usato
-- a_data_scad_saldo      IN date, -- non usato
...
BEGIN
  ...
  w_data_scad_acconto := f_scadenza(a_anno,'ICI', 'A', rec_riog.cod_fiscale);
  w_data_scad_saldo  := f_scadenza(a_anno,'ICI', 'A', rec_riog.cod_fiscale);
  w_mError := '';
  wf := 1;
    BEGIN
    ...
    END;
   --      w_perc_diff := round(rec_riog.valore * 100 / decode(rec_riog.valore_dic,
   --                     0,1,
   --                     nvl(rec_riog.valore_dic,1)),2) - 100;
    ...
    ELSE
      ...
      IF w_controllo is null THEN
        ...
        BEGIN
          ...
          values (w_pratica,rec_riog.cod_fiscale,'ICI',a_anno,
...
--         and w_max_al_riog  >= to_date('3112'||lpad(to_char(w_anno),4,'0'),'ddmmyyyy')
         ...
         EXCEPTION
          WHEN no_data_found THEN
            null;
          WHEN others THEN
            w_errore := 'Errore in verifica versamenti ici (riog) ';
            RAISE errore;
         END;
         -- Versamenti su ravvediemnto
         begin
           w_versamenti_ravv       := F_IMPORTO_VERS_RAVV(rec_riog.cod_fiscale,'ICI',a_anno,'U');
           w_versamenti_ravv_acc   := F_IMPORTO_VERS_RAVV(rec_riog.cod_fiscale,'ICI',a_anno,'A');
         end;
        ...
        wf := 12;
         IF nvl(rec_riog.detrazione,0) != 0 THEN
            ...
            END;
            --             w_detr_acconto :=  f_round(nvl(rec_riog.imposta_acconto,0) * nvl(rec_riog.detrazione,0)
            --            / decode(rec_riog.imposta,0,1,nvl(rec_riog.imposta,1)),0);
         END IF;
         ...
         EXCEPTION
               WHEN others THEN
                 w_errore := 'Errore in totalizz. importo (old) ';
                 RAISE errore;
         END;
         wf := 21;
         BEGIN
         ...
         IF (w_imp_liq_new = 0 AND w_imp_liq_old = 0)
             OR a_anno >= 1998 AND w_imp_liq_new not between nvl(a_importo_limite_inf,-999999999)
                                    and nvl(a_importo_limite_sup,999999999)
             OR a_anno < 1998 AND w_imp_liq_old not between nvl(a_importo_limite_inf,-999999999)
         ...
         ELSIF w_imp_liq_new > w_imp_liq_old and a_anno < 1998 THEN
            ...
            EXCEPTION
              WHEN others THEN
                w_errore := 'Errore in cancellaz. sanz. pratica new ';
                RAISE errore;
            END;
         ELSIF w_imp_liq_new <= w_imp_liq_old and a_anno >= 1998 THEN
            wf := 24;
            BEGIN
              ...
              IF f_check_sanzione(w_pratica, w_cod_sanzione) = 0 THEN
...
EXCEPTION
  ...
  WHEN others THEN
       ROLLBACK;
       RAISE_APPLICATION_ERROR (-20999,'Errore in LIQUIDAZIONI_ICI_RIOG -'||
                               to_char(wf)||'- ('||SQLERRM||')');
END;
/* End Procedure: LIQUIDAZIONI_ICI_RIOG */
/
