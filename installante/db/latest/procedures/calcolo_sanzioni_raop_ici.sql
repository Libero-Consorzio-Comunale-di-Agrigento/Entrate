...
--changeset abrandolini:20250326_152423_calcolo_sanzioni_raop_ici stripComments:false runOnChange:true 
 
CREATE OR REPLACE procedure     CALCOLO_SANZIONI_RAOP_ICI
/*************************************************************************
 Versione  Data              Autore    Descrizione
 12        08/11/2024        DM        #75093
                                       Storicizzazione sanzioni
 11        19/06/2024        RV        #73285
                                       Sistemazione calcolo interessi eccedenza in acconto
 ...
 8         16/12/2022        AB        aggiornato imposta, imposta_acc,
                                       imposta_sal come somma dei dettagli
                                       arrotondati
 7         03/08/2021        VD        Aggiunto arrotonamento imposta per
                                       evitare l'emissione del ravvedimento
                                       in caso di differenza dovute ai
                                       versamenti arrotondati.
 6         22/09/2020        VD        Aggiunta gestione fabbricati merce
 5         05/02/2020        VD        Nuovo sanzionamento per ravvedimento
                                       operoso "lungo"
 4         01/09/2016        VD        Corretta gestione riduzione aliquota
                                       per sanzione 513
                                       (riduzione corretta 1/8)
 3         25/07/2016        VD        Gestione ravvedimento mini IMU
 2         18/01/2016        VD        Modificata gestione sanzioni per il
...
-- IMU --
  ...
  -- Mini IMU
  w_scadenza_mini                date;
  w_diff_giorni_mini             number;
  w_imposta_mini                 number;
  w_imposta_ab_mini              number;
  w_imposta_ter_mini             number;
  w_versato_mini                 number;
  w_versato_ab_mini              number;
  w_versato_ter_mini             number;
  w_versato_tard_mini            number;
  w_versato_tard_ab_mini         number;
  w_versato_tard_ter_mini        number;
-- Fabbricati Merce
  w_imposta_fabb_merce           number;
  w_imposta_fabb_merce_acc       number;
  w_imposta_fabb_merce_sal       number;
  w_n_fab_fabb_merce             number;
  w_versato_fabb_merce_acc       number;
  w_versato_fabb_merce_sal       number;
  w_versato_tard_fabb_merce_acc  number;
  w_versato_tard_fabb_merce_sal  number;
  w_sanzione_fabb_merce          number;
-----------------------------------------
  ...
  EXCEPTION
     when no_data_found then
        if a_tipo_scad = 'V' then
           w_err := 'Scadenza di pagamento ICI ';
           ...
           w_err := 'Scadenza di presentazione denuncia ICI non prevista per anno '||
                    to_char(a_anno);
        end if;
        Return w_err;
     WHEN others THEN
...
, a_importo_fabb_merce  in     number
, a_data_scadenza       in     date
...
, a_sanzione_fabb_merce in out number
...
BEGIN
  ...
  if a_anno < 2012 then
    ...
    -- IMU --
     ...
     a_sanzione_fabb_merce := f_round((a_importo_fabb_merce * nvl(a_percentuale,0)) / 100,0);
...
, a_interessi_fabb_merce in out number
...
IS
  ...
  w_interesse_singolo_fabb_merce number;
...
BEGIN
  ...
  begin
     ...
     loop
        if a_anno < 2012 then
          ...
          -- IMU --
           ...
           w_interesse_singolo_fabb_merce := f_round(nvl(a_importo_fabb_merce,0) * nvl(rec_periodo.aliquota,0) / 100 *
                                          (rec_periodo.al - rec_periodo.dal + 1) / a_gg_anno,0
                                         );
           w_interessi_fabb_merce := w_interessi_fabb_merce + w_interesse_singolo_fabb_merce;
           w_interesse_singolo := w_interesse_singolo_ab + w_interesse_singolo_rur
                                ...
                                + w_interesse_singolo_fabb_merce;
           w_interessi    := w_interessi + w_interesse_singolo;
           w_note_singolo := 'Int: '||to_char(w_interesse_singolo)
                          ||' gg: '||to_char((rec_periodo.al - rec_periodo.dal + 1))
                          ||' dal: '||to_char(rec_periodo.dal,'dd/mm/yyyy')
                          ...
                          ||' imp_fabb_merce: '||to_char(nvl(a_importo_fabb_merce,0))
                          ||' - ';
           w_note := w_note || w_note_singolo;
        end if;
     end loop;
     ...
     a_interessi_fabb_merce := f_round(w_interessi_fabb_merce,0);
     a_note                 := w_note;
  EXCEPTION
     WHEN others THEN
        w_err := to_char(SQLCODE)||' - '||SQLERRM;
...
-- nel calcolo degli interessi al denominatore si utilizza sempre 365 anche
-- in caso di anni bisestili (27/07/2009)
   w_giorni_anno := 365;
   w_errore := null;
...
--
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         ...
         RAISE ERRORE;
   END;
   -- setto a 1 la variabile w_se_anno_fabb_d per sapere se nell'anno della pratica
   -- è prevista la gestione dei fabbricati D (fabbricati ad uso produttivo)
   if w_anno >= 2013 then
      w_se_anno_fabb_d := 1;
   else
      w_se_anno_fabb_d := 0;
...
--
-- Determinazione scadenza mini IMU (solo per anno 2013)
--
  if w_anno = 2013 then
     w_scadenza_mini := F_SCADENZA_MINI_IMU(w_anno,a_pratica);
  else
     w_scadenza_mini := to_date(null);
  end if;
--
...
--
-- 18/10/2013 - x cambio di legislazione, in regime di IMU, la data di scadenza
-- del ravvedimento è sempre 90 giorni dopo la data di scadenza della denuncia per l'anno
--
  --
  ...
  --
       ...
       where sapr.pratica                      = prtr.pratica
         ...
         and (    a_tipo_pagam                 = 'A'
              ...
              and sapr.cod_sanzione           in (153,154,156,159,160,167,168,
                                                  511,512,513,514)
              or  a_tipo_pagam                 = 'U'
              and sapr.cod_sanzione           in (151,152,153,154,155,156,157,158,159,160,
                                                  165,166,167,168,
                                                  511,512,513,514)
             )
      ;
   ...
   BEGIN
      select nvl(sum(nvl(ogim.imposta_acconto,0)),0)
            ,nvl(sum(nvl(ogim.imposta,0) - nvl(ogim.imposta_acconto,0) - nvl(ogim.imposta_mini,0)),0)
            ,nvl(sum(nvl(ogim.imposta,0)),0)
            ,nvl(sum(nvl(ogim.imposta_mini,0)),0)
        into w_imposta_acc
            ,w_imposta_sal
            ,w_imposta
            ,w_imposta_mini
...
--
      ...
      select sum(decode(ogim.tipo_aliquota
           ...
           , sum(decode(ogim.tipo_aliquota
                       ...
                       ,decode(nvl(ogpr.tipo_oggetto,ogge.tipo_oggetto)
                              ,1,0
                              ,2,0
                              ,decode(aliq.flag_fabbricati_merce
                                     ,'S',0
           ...
           , sum(decode(aliq.flag_fabbricati_merce
           ...
           , sum(decode(aliq.flag_fabbricati_merce
                ...
                )                                                               imposta_fabb_merce_acc
           , sum(decode(aliq.flag_fabbricati_merce
                       ,'S',nvl(ogim.imposta,0) - nvl(ogim.imposta_acconto,0)
                       ,0
                       )
                )                                                               imposta_fabb_merce_sal
           , sum(decode(aliq.flag_fabbricati_merce
                       ,'S',1
                       ,0
                       )
        ...
        into w_imposta_ab
           ...
           , w_imposta_ab_mini
           , w_imposta_ter_mini
           , w_imposta_fabb_merce
           , w_imposta_fabb_merce_acc
           , w_imposta_fabb_merce_sal
           , w_n_fab_fabb_merce
        from oggetti_imposta  ogim
           , oggetti_pratica  ogpr
           , oggetti          ogge
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         ...
         w_n_fab_fabb_d                 := 0;
         w_imposta_ab_mini              := 0;
         w_imposta_ter_mini             := 0;
         w_imposta_fabb_merce           := 0;
         w_imposta_fabb_merce_acc       := 0;
         w_imposta_fabb_merce_sal       := 0;
         w_n_fab_fabb_merce             := 0;
   END;
   ...
   -- Mini IMU: si decurta l'imposta a saldo dell'eventuale mini IMU
   w_imposta_ab_sal      := w_imposta_ab_sal      - w_imposta_ab_mini;
   w_imposta_ter_com_sal := w_imposta_ter_com_sal - w_imposta_ter_mini;
...
--                         che non vengono mai utilizzate ...
   ...
   w_imposta_ter_com              := round(w_imposta_ter_com,0);
   w_imposta_ter_com_acc          := round(w_imposta_ter_com_acc,0);
   w_imposta_ter_com_sal          := round(w_imposta_ter_com_sal,0);
   w_imposta_ter_sta              := round(w_imposta_ter_sta,0);
   w_imposta_ter_sta_acc          := round(w_imposta_ter_sta_acc,0);
   w_imposta_ter_sta_sal          := round(w_imposta_ter_sta_sal,0);
   w_imposta_aree_com             := round(w_imposta_aree_com,0);
   w_imposta_aree_com_acc         := round(w_imposta_aree_com_acc,0);
   w_imposta_aree_com_sal         := round(w_imposta_aree_com_sal,0);
   w_imposta_aree_sta             := round(w_imposta_aree_sta,0);
   w_imposta_aree_sta_acc         := round(w_imposta_aree_sta_acc,0);
   w_imposta_aree_sta_sal         := round(w_imposta_aree_sta_sal,0);
   w_imposta_altri_com            := round(w_imposta_altri_com,0);
   w_imposta_altri_com_acc        := round(w_imposta_altri_com_acc,0);
   w_imposta_altri_com_sal        := round(w_imposta_altri_com_sal,0);
   w_imposta_altri_sta            := round(w_imposta_altri_sta,0);
   w_imposta_altri_sta_acc        := round(w_imposta_altri_sta_acc,0);
   w_imposta_altri_sta_sal        := round(w_imposta_altri_sta_sal,0);
   w_imposta_fabb_d_com           := round(w_imposta_fabb_d_com,0);
   w_imposta_fabb_d_com_acc       := round(w_imposta_fabb_d_com_acc,0);
   w_imposta_fabb_d_com_sal       := round(w_imposta_fabb_d_com_sal,0);
   w_imposta_fabb_d_sta           := round(w_imposta_fabb_d_sta,0);
   w_imposta_fabb_d_sta_acc       := round(w_imposta_fabb_d_sta_acc,0);
   w_imposta_fabb_d_sta_sal       := round(w_imposta_fabb_d_sta_sal,0);
   w_imposta_ab_mini              := round(w_imposta_ab_mini,0);
   w_imposta_ter_mini             := round(w_imposta_ter_mini,0);
   w_imposta_fabb_merce           := round(w_imposta_fabb_merce,0);
   w_imposta_fabb_merce_acc       := round(w_imposta_fabb_merce_acc,0);
   w_imposta_fabb_merce_sal       := round(w_imposta_fabb_merce_sal,0);
   w_imposta_acc                      := nvl(w_imposta_ab_acc,0)
                                       + nvl(w_imposta_ter_com_acc,0)
                                       + nvl(w_imposta_ter_sta_acc,0)
                                       + nvl(w_imposta_aree_com_acc,0)
                                       + nvl(w_imposta_aree_sta_acc,0)
                                       + nvl(w_imposta_rur_acc,0)
                                       + nvl(w_imposta_fabb_d_com_acc,0)
                                       + nvl(w_imposta_fabb_d_sta_acc,0)
                                       + nvl(w_imposta_altri_com_acc,0)
                                       + nvl(w_imposta_altri_sta_acc,0)
                                       + nvl(w_imposta_fabb_merce_acc,0);
   w_imposta                          := nvl(w_imposta_ab,0)
                                       + nvl(w_imposta_ter_com,0)
                                       + nvl(w_imposta_ter_sta,0)
                                       + nvl(w_imposta_aree_com,0)
                                       + nvl(w_imposta_aree_sta,0)
                                       + nvl(w_imposta_rur,0)
                                       + nvl(w_imposta_fabb_d_com,0)
                                       + nvl(w_imposta_fabb_d_sta,0)
                                       + nvl(w_imposta_altri_com,0)
                                       + nvl(w_imposta_altri_sta,0)
                                       + nvl(w_imposta_fabb_merce,0); -- AB aggiunto il 15/12/2022, ma non sono convinto...
    w_imposta_sal                     := nvl(w_imposta_ab_sal,0)
                                       + nvl(w_imposta_ter_com_sal,0)
                                       + nvl(w_imposta_ter_sta_sal,0)
                                       + nvl(w_imposta_aree_com_sal,0)
                                       + nvl(w_imposta_aree_sta_sal,0)
                                       + nvl(w_imposta_rur_sal,0)
                                       + nvl(w_imposta_fabb_d_com_sal,0)
                                       + nvl(w_imposta_fabb_d_sta_sal,0)
                                       + nvl(w_imposta_altri_com_sal,0)
                                       + nvl(w_imposta_altri_sta_sal,0)
                                       + nvl(w_imposta_fabb_merce_sal,0);
   --dbms_output.put_line('1) - Imposta '||w_imposta||' Saldo '||w_imposta_sal||
   --                     ' Acconto '||w_imposta_acc||' Mini '||w_imposta_mini);
   BEGIN
      select nvl(sum(decode(vers.tipo_versamento
           ...
           , nvl(sum(decode(vers.tipo_versamento
                           ,'S',0
                           ,'A',nvl(vers.altri_comune ,0)
                               ,decode(sign(nvl(vers.altri_comune,0) - w_imposta_altri_com_acc)
                                      ,-1,nvl(vers.altri_comune,0)
                                         ,w_imposta_altri_com_acc
                                      )
                           )
                    ),0
            ...
            ,nvl(sum(decode(vers.tipo_versamento
                           ...
                           ,'A',decode(sign(vers.data_pagamento - w_scadenza_acc)
                               ...
                               ,decode(sign(vers.data_pagamento - w_scadenza_acc)
                                      ,1,decode(sign(nvl(vers.altri_comune,0) -
                                                     w_imposta_altri_com_acc
                                                    )
                                               ,-1,nvl(vers.altri_comune,0)
                                                  ,0
           ...
           , nvl(sum(decode(vers.tipo_versamento
            ...
            ,nvl(sum(decode(vers.anno
                           ,2013,decode(vers.tipo_versamento
                                       ,'U',nvl(vers.importo_versato,0)
                                           ,0
                                       )
                                ,0
                           )
            ...
            ,nvl(sum(decode(vers.anno
                           ,2013,decode(vers.tipo_versamento
                                       ,'U',decode(sign(vers.data_pagamento - w_scadenza_mini)
                                                  ,1,nvl(vers.importo_versato,0)
                                                    ,0
                                                  )
                                           ,0
            ...
            ,nvl(sum(decode(vers.anno
                ...
                )                                                               w_versato_tard_ter_mini
           , nvl(sum(decode(vers.tipo_versamento
                           ,'S',0
                           ,'A',nvl(vers.fabbricati_merce,0)
                               ,decode(sign(nvl(vers.fabbricati_merce,0) - w_imposta_fabb_merce_acc)
                                      ,-1,nvl(vers.fabbricati_merce,0)
                                         ,w_imposta_fabb_merce_acc
                                      )
                           )
                    ),0
                )                                                               w_versato_fabb_merce_acc
            ,nvl(sum(decode(vers.tipo_versamento
                           ,'A',0
                           ,'S',nvl(vers.fabbricati_merce,0)
                               ,decode(sign(nvl(vers.fabbricati_merce,0) - w_imposta_fabb_merce_acc)
                                      ,-1,0
                                         ,nvl(vers.fabbricati_merce,0) - w_imposta_fabb_merce_acc
                                      )
                           )
                    ),0
                )                                                               versato_fabb_merce_sal
            ,nvl(sum(decode(vers.tipo_versamento
                           ,'S',0
                           ,'A',decode(sign(vers.data_pagamento - w_scadenza_acc)
                                      ,1,nvl(vers.fabbricati_merce,0)
                               ...
                               ,decode(sign(vers.data_pagamento - w_scadenza_acc)
                                      ,1,decode(sign(nvl(vers.fabbricati_merce,0) -
                                                     w_imposta_fabb_merce_acc
                                                    )
                                               ,-1,nvl(vers.fabbricati_merce,0)
                                                  ,0
        ...
        into w_versato_acc
           ...
           , w_versato_ter_mini
           , w_versato_tard_mini
           , w_versato_tard_ab_mini
           , w_versato_tard_ter_mini
           , w_versato_fabb_merce_acc
           , w_versato_fabb_merce_sal
           , w_versato_tard_fabb_merce_acc
           , w_versato_tard_fabb_merce_sal
       ...
       where vers.tipo_tributo          = 'ICI'
         ...
         and ((    a_tipo_pagam         = 'A'
              ...
              or
              -- #73285 : in caso di saldo contabilizziamo pure i versamenti in acconto,
              --          altrimentio perdiamo l'extraversato
              --          Prima prendeva solo i versamenti coincidenti con la rata
              (    a_tipo_pagam         = 'S'
               and vers.tipo_versamento in ('A','S'))
            -- and a_tipo_pagam = vers.tipo_versamento)    -- #73285
              or
              (    a_tipo_pagam         = 'U'
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         ...
         w_versato_mini                := 0;
         w_versato_ab_mini             := 0;
         w_versato_ter_mini            := 0;
         w_versato_tard_mini           := 0;
         w_versato_tard_ab_mini        := 0;
         w_versato_tard_ter_mini       := 0;
         w_versato_fabb_merce_acc      := 0;
         w_versato_fabb_merce_sal      := 0;
         w_versato_tard_fabb_merce_acc := 0;
         w_versato_tard_fabb_merce_sal := 0;
   END;
   --
   -- Mini IMU: se il versato mini (tipo versamento = U) e maggiore
   --           della mini IMU, si considera = alla mini IMU e il resto
   --           rimane come versamento in acconto
   --
   if w_imposta_mini > 0 then
      if w_versato_mini > w_imposta_mini then
         w_versato_mini := w_imposta_mini;
      end if;
      if w_versato_acc >= w_versato_mini then
         w_versato_acc := w_versato_acc - w_versato_mini;
         w_versato_tard_acc := w_versato_tard_acc - w_versato_mini;
      end if;
      if w_versato_sal >= w_versato_mini then
         w_versato_sal := w_versato_sal - w_versato_mini;
         w_versato_tard_sal := w_versato_tard_sal - w_versato_mini;
      end if;
      if w_versato_ab_mini > w_imposta_ab_mini then
         w_versato_ab_mini := w_imposta_ab_mini;
      end if;
      if w_versato_ab_acc >= w_versato_ab_mini then
         w_versato_ab_acc := w_versato_ab_acc - w_versato_ab_mini;
         w_versato_tard_ab_acc := w_versato_tard_ab_acc - w_versato_ab_mini;
      end if;
      if w_versato_ab_sal >= w_versato_ab_mini then
         w_versato_ab_sal := w_versato_ab_sal - w_versato_ab_mini;
         w_versato_tard_ab_sal := w_versato_tard_ab_sal - w_versato_ab_mini;
      end if;
      if w_versato_ter_mini > w_imposta_ter_mini then
         w_versato_ter_mini := w_imposta_ter_mini;
      end if;
      if w_versato_ter_com_acc >= w_versato_ter_mini then
         w_versato_ter_com_acc := w_versato_ter_com_acc - w_versato_ter_mini;
         w_versato_tard_ter_com_acc := w_versato_tard_ter_com_acc - w_versato_ter_mini;
      end if;
      if w_versato_ter_com_sal >= w_versato_ter_mini then
         w_versato_ter_com_sal := w_versato_ter_com_sal - w_versato_ter_mini;
         w_versato_tard_ter_com_sal := w_versato_tard_ter_com_sal - w_versato_ter_mini;
      end if;
   else
      w_versato_mini     := 0;
      w_versato_ab_mini  := 0;
   ...
   --
   -- Mini IMU: se il versato tardivo mini (tipo versamento = U) e maggiore
   --           della mini IMU, si considera = alla mini IMU e il resto
   --           rimane come tardivo versamento in acconto
   --
   --    dbms_output.put_line('*) Versato - Acconto '||to_char(w_versato_acc)||' Saldo '||
   --                     to_char(w_versato_sal)||' Mini '||to_char(w_versato_mini));
   --    dbms_output.put_line('*) Tardivo - Acconto '||to_char(w_versato_tard_acc)||' Saldo '||
   --                     to_char(w_versato_tard_sal)||' Mini '||to_char(w_versato_tard_mini));
   if w_imposta_mini > 0 then
      if w_versato_tard_mini > w_imposta_mini then
         w_versato_tard_mini := w_imposta_mini;
      end if;
      if w_versato_tard_acc >= w_versato_tard_mini then
         w_versato_tard_acc := w_versato_tard_acc - w_versato_tard_mini;
         if w_versato_tard_mini = 0 then
            w_versato_tard_acc := greatest(0,w_versato_tard_acc - w_versato_mini);
         end if;
      end if;
      if w_versato_tard_ab_mini > w_imposta_ab_mini then
         w_versato_tard_ab_mini := w_imposta_ab_mini;
      end if;
      if w_versato_tard_ab_acc >= w_versato_tard_ab_mini then
         w_versato_tard_ab_acc := w_versato_tard_ab_acc - w_versato_tard_ab_mini;
         if w_versato_tard_ab_mini = 0 then
            w_versato_tard_ab_acc := greatest(0,w_versato_tard_ab_acc - w_versato_ab_mini);
         end if;
      end if;
      if w_versato_tard_ter_mini > w_imposta_ter_mini then
         w_versato_tard_ter_mini := w_imposta_ter_mini;
      end if;
      if w_versato_tard_ter_com_acc >= w_versato_ter_mini then
         w_versato_tard_ter_com_acc := w_versato_tard_ter_com_acc - w_versato_tard_ter_mini;
         if w_versato_tard_ter_mini = 0 then
            w_versato_tard_ter_com_acc := greatest(0,w_versato_tard_ter_com_acc - w_versato_ter_mini);
         end if;
      end if;
   else
      w_versato_tard_mini     := 0;
   ...
   --    dbms_output.put_line('X) Versato - Acconto '||to_char(w_versato_acc)||' Saldo '||
   --                     to_char(w_versato_sal)||' Mini '||to_char(w_versato_mini));
   --    dbms_output.put_line('X) Tardivo - Acconto '||to_char(w_versato_tard_acc)||' Saldo '||
   --                     to_char(w_versato_tard_sal)||' Mini '||to_char(w_versato_tard_mini));
   ...
   if w_versato_fabb_merce_acc > w_imposta_fabb_merce_acc then
      w_versato_fabb_merce_sal := w_versato_fabb_merce_sal + (w_versato_fabb_merce_acc - w_imposta_fabb_merce_acc);
   end if;
   --
   -- Mini IMU: l'eventuale versamento a saldo in eccesso viene
   --           attribuito alla mini IMU
   --
   if w_imposta_mini > 0 then
      if w_versato_sal > w_imposta_sal then
         if (w_versato_sal - w_imposta_sal) > w_imposta_mini then
            w_versato_sal  := w_versato_sal - w_imposta_mini;
            w_imposta_mini := 0;
         else
            w_imposta_mini := w_imposta_mini - (w_versato_sal - w_imposta_sal);
            w_versato_sal  := w_imposta_sal;
         end if;
      end if;
      if w_versato_ab_sal > w_imposta_ab_sal then
         if (w_versato_ab_sal - w_imposta_ab_sal) > w_imposta_ab_mini then
            w_versato_ab_sal  := w_versato_ab_sal - w_imposta_ab_mini;
            w_imposta_ab_mini := 0;
         else
            w_imposta_ab_mini := w_imposta_ab_mini - (w_versato_ab_sal - w_imposta_ab_sal);
            w_versato_ab_sal  := w_imposta_ab_sal;
         end if;
      end if;
      if w_versato_ter_com_sal > w_imposta_ter_com_sal then
         if (w_versato_ter_com_sal - w_imposta_ter_com_sal) > w_imposta_ter_mini then
            w_versato_ter_com_sal := w_versato_ter_com_sal - w_imposta_ter_mini;
            w_imposta_ter_mini    := 0;
         else
            w_imposta_ter_mini    := w_imposta_ter_mini - (w_versato_ter_com_sal - w_imposta_ter_com_sal);
            w_versato_ter_com_sal := w_imposta_ter_com_sal;
         end if;
      end if;
   end if;
   --    dbms_output.put_line('1) Versato - Acconto '||to_char(w_versato_acc)||' Saldo '||
   --                     to_char(w_versato_sal)||' Mini '||to_char(w_versato_mini));
   --    dbms_output.put_line('1) Tardivo - Acconto '||to_char(w_versato_tard_acc)||' Saldo '||
   --                     to_char(w_versato_tard_sal)||' Mini '||to_char(w_versato_tard_mini));
   ...
   if w_versato_mini > w_imposta_mini then
      ...
      w_imposta_mini := w_imposta_mini - w_versato_mini;
   end if;
   if w_versato_ab_mini > w_imposta_ab_mini then
      w_imposta_ab_mini := 0;
   else
      w_imposta_ab_mini := w_imposta_ab_mini - w_versato_ab_mini;
   end if;
   if w_versato_ter_mini > w_imposta_ter_mini then
      w_imposta_ter_mini := 0;
   else
      w_imposta_ter_mini := w_imposta_ter_mini - w_versato_ter_mini;
   end if;
   if a_tipo_pagam in ('A','U') then
      if w_versato_acc > w_imposta_acc then
         w_versato_acc := w_versato_acc - w_imposta_acc;
         w_imposta_acc := 0;
      else
   ...
   if a_tipo_pagam in ('S','U') then
      if w_versato_sal > w_imposta_sal then
         w_versato_sal := w_versato_sal - w_imposta_sal;
         w_imposta_sal := 0;
      else
   ...
   --    dbms_output.put_line('2) Versato - Acconto '||to_char(w_versato_acc)||' Saldo '||
   --                     to_char(w_versato_sal)||' Mini '||to_char(w_versato_mini));
   --    dbms_output.put_line('2) Tardivo - Acconto '||to_char(w_versato_tard_acc)||' Saldo '||
   --                     to_char(w_versato_tard_sal)||' Mini '||to_char(w_versato_tard_mini));
   --    dbms_output.put_line('Non Versato - Acconto '||to_char(w_imposta_acc)||' Saldo '||
   --                     to_char(w_imposta_sal)||' Mini '||to_char(w_imposta_mini));
   --
   if a_tipo_pagam in ('A','U') then
      if w_versato_ab_acc > w_imposta_ab_acc then
         w_imposta_ab_acc := 0;
   ...
   -- (VD - 22/09/2020): aggiunta gestione fabbricati merce
   if a_tipo_pagam in ('A','U') then
      if w_versato_fabb_merce_acc > w_imposta_fabb_merce_acc then
         w_imposta_fabb_merce_acc := 0;
      else
         w_imposta_fabb_merce_acc := w_imposta_fabb_merce_acc - w_versato_fabb_merce_acc;
      end if;
   end if;
   if a_tipo_pagam in ('S','U') then
      if w_versato_fabb_merce_sal > w_imposta_fabb_merce_sal then
         w_imposta_fabb_merce_sal := 0;
      else
         w_imposta_fabb_merce_sal := w_imposta_fabb_merce_sal - w_versato_fabb_merce_sal;
      end if;
   end if;
   w_imposta_fabb_merce := w_imposta_fabb_merce_acc + w_imposta_fabb_merce_sal;
--
-- Mini IMU: eventuali versamenti residui di tipo diverso da "U" vengono
--           spalmanti sulla mini IMU
--
   if w_imposta_mini > 0 then
      if w_versato_acc > w_imposta_mini then
         w_versato_acc  := w_versato_acc - w_imposta_mini;
         w_imposta_mini := 0;
      else
         w_imposta_mini := w_imposta_mini - w_versato_acc;
         w_versato_acc  := 0;
      end if;
      if w_versato_sal > w_imposta_mini then
         w_versato_sal  :=  w_versato_sal - w_imposta_mini;
         w_imposta_mini := 0;
      else
         w_imposta_mini := w_imposta_mini - w_versato_sal;
         w_versato_sal  := 0;
      end if;
   end if;
   if w_imposta_ab_mini > 0 then
      if w_versato_ab_acc > w_imposta_ab_mini then
         w_versato_ab_acc  := w_versato_ab_acc - w_imposta_ab_mini;
         w_imposta_ab_mini := 0;
      else
         w_imposta_ab_mini := w_imposta_ab_mini - w_versato_ab_acc;
         w_versato_ab_acc  := 0;
      end if;
      if w_versato_ab_sal > w_imposta_ab_mini then
         w_versato_ab_sal  :=  w_versato_ab_sal - w_imposta_ab_mini;

         w_imposta_ab_mini := 0;
      else
         w_imposta_ab_mini := w_imposta_ab_mini - w_versato_ab_sal;
         w_versato_ab_sal  := 0;
      end if;
   end if;
   if w_imposta_ter_mini > 0 then
      if w_versato_ter_com_acc > w_imposta_ter_mini then
         w_versato_ter_com_acc  := w_versato_ter_com_acc - w_imposta_ter_mini;
         w_imposta_ter_mini     := 0;
      else
         w_imposta_ter_mini     := w_imposta_ter_mini - w_versato_ter_com_acc;
         w_versato_ter_com_acc  := 0;
      end if;
      if w_versato_ter_com_sal > w_imposta_ter_mini then
         w_versato_ter_com_sal  :=  w_versato_ter_com_sal - w_imposta_ter_mini;
         w_imposta_ter_mini     := 0;
      else
         w_imposta_ter_mini     := w_imposta_ter_mini - w_versato_ter_com_sal;
         w_versato_ter_com_sal  := 0;
      end if;
   end if;
   --
   ...
   if a_flag_infrazione in ('O','I') then
      ...
      values( a_pratica, w_cod_sanzione, 'ICI'
   ...
   if a_tipo_pagam in ('A','U') and w_diff_giorni_acc > 0 and w_imposta_acc > 0 then
      w_cod_sanzione := 101;
--       w_errore := 'Sanzione '||w_cod_sanzione||' - '||w_imposta_acc||' altri_com '||w_imposta_altri_com_acc||SQLERRM;
--                             RAISE ERRORE;

       w_errore := F_SEQUENZA_SANZIONE(w_cod_sanzione,w_scadenza_acc,w_sequenza_sanz);
       if w_errore is not null then
           RAISE ERRORE;
       end if;
   ...
   if a_tipo_pagam in ('S','U') and w_diff_giorni > 0 and w_imposta_sal > 0 then
   ...
   -- (VD - 05/02/2020): aggiunta gestione ravvedimento lungo
   --
   if w_diff_giorni_acc > 0 then
      ...
      if w_diff_giorni_acc > 730 and
         ...
         if w_anno > 1999 then
            if w_scadenza_acc > to_date('31/01/2011','dd/mm/yyyy') then
               w_rid := 8;
            elsif a_data_pagam >= to_date('29/11/2008','dd/mm/yyyy') then
               w_rid := 10;
            else
      ...
      if a_tipo_pagam in ('A','U') and (w_versato_tard_acc > 0 or w_imposta_acc > 0) then
         ...
         w_errore := F_SEQUENZA_SANZIONE(w_cod_sanzione,w_data_pratica,w_sequenza_sanz);
         if w_errore is not null then
           RAISE ERRORE;
         end if;
         ...
         values( a_pratica, w_cod_sanzione,'ICI'
               ...
               , w_sanzione_fabb_d_com, w_sanzione_fabb_d_sta
               , w_sanzione_fabb_merce
               , w_giorni, a_utente, trunc(sysdate), w_note_sanzione, w_sequenza_sanz
               )
         ;
      end if;
   end if;
...
END;
/* End Procedure: CALCOLO_SANZIONI_RAOP_ICI */
/
