...
--changeset abrandolini:20250326_152423_calcolo_importo_normalizzato stripComments:false runOnChange:true 
 
create or replace procedure CALCOLO_IMPORTO_NORMALIZZATO
/******************************************************************************
 Rev. Data       Autore Descrizione
 ---- ---------- ------ -----------------------------------------------------
  5   30/10/2018 VD     Aggiunto flag gestione importi calcolati con tariffa
                        base (utilizzato solo per la composizione delle
                        stringhe contenenti le informazioni sul calcolo).
  4   27/04/2017 AB     Aggiunta la gestione della percentuale nei ruoli di
                        acconto. Si utilizza il nuovo campo perc_acconto,
                        oppure cod_sede per chi non ha ancora la from nuova
  3   18/11/2016 VD     San Donato Milanese - Sperimentazione Poasco
                        Applicazione degli sconti per conferimento sacchi
                        Nota sulla composizione di dettaglio_ogim:
                        la stringa relativa allo sconto conferimenti viene
                        compilata solamente se si verificano le seguenti
                        condizioni:
                        - cliente S.Donato Milanese
                        - ruolo suppletivo totale
                        - utenza domestica
                        - zona territoriale della sperimentazione
                        - percentuale sconto non nulla
                        In tutto gli altri casi la stringa rimane invariata.
                        In questo modo, il test sulla lunghezza della stringa
                        che serve a determinare se si registra tutto su OGIM
                        o se bisogna registrare le varie stringhe spezzettate
                        su FAOG viene modificato solo nella procedure
                        EMISSIONE_RUOLO, caso ruolo suppletivo totale (dove
                        la procedure viene richiamata passando l'oggetto).
  2   06/07/2016 AB     Gestito w_gg_anno contentente i gg dell'anno
                        sulla base dell'ultimo giorno del mese di Febbraio
  1   08/05/2001 DM     Gestiti i Periodi di Familiari Soggetti intersecantisi
                        con i periodi di Oggetti Validita
  0   26/06/2000        Prima emissione
******************************************************************************/
...
,a_tributo                number
,a_categoria              number
,a_tipo_tariffa           number --MAI USATO
,a_tariffa                number
...
,a_flag_tariffa_base      varchar2 default null
) IS
errore                    exception;
...
w_gg_anno                 number;
--
-- (VD - 18/11/2016): Dati per sperimentazione Poasco
--
w_unita_terr              number;
         ...
         LOOP
            ...
            if w_giro = 1 then
               ...
               if w_numero_familiari = rec_faso.numero_familiari
                  ...
                  --
                  -- (VD - 30/10/2018): modificata composizione stringa per
                  ...
                  -- (VD - 30/10/2018): modificata composizione stringa per
                  --                    importi calcolati con tariffa base
                  if a_flag_tariffa_base is null then
                     w_dettaglio_pf := ' Coeff. '||lpad(nvl(translate(ltrim(to_char(w_coeff1,'90.0000')),'.,',',.'),' '),7,' ')
                                    ||' Tariffa ' ||lpad(nvl(translate(ltrim(to_char(w_tari,'999,990.00000')),'.,',',.'),' '),13,' ')
                                    ||' Imposta QF' ||lpad(nvl(translate(ltrim(to_char(w_dep_imp_pf,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
                     ...
                     w_dettaglio_pf := ' Coeff. '||lpad(nvl(translate(ltrim(to_char(w_coeff1,'90.0000')),'.,',',.'),' '),7,' ')
                                    ||' Tariffa Base ' ||lpad(nvl(translate(ltrim(to_char(w_tari,'999,990.00000')),'.,',',.'),' '),13,' ')
                                    ||' Imposta Base QF' ||lpad(nvl(translate(ltrim(to_char(w_dep_imp_pf,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
                     w_dettaglio_pv := ' Coeff. '||lpad(nvl(translate(ltrim(to_char(w_coeff2,'90.0000')),'.,',',.'),' '),7,' ')
                                    ||' Tariffa Base ' ||lpad(nvl(translate(ltrim(to_char(a_tariffa,'999,990.00000')),'.,',',.'),' '),13,' ')
                                    ||' Imposta Base QV' ||lpad(nvl(translate(ltrim(to_char(w_dep_imp_pv,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
                  end if;
                  --
                  -- (VD - 18/11/2016) - Sperimentazione Poasco
...
--' imp.tot '||to_char(w_importo));
         ...
         w_importo_pv := w_importo_pv + w_dep_imp_pv;
         w_stringa_familiari := w_stringa_familiari || lpad(to_char(nvl(w_numero_familiari,0)),4,'0') || to_char(w_dal,'ddmmyyyy')|| to_char(w_al,'ddmmyyyy');
--DBMS_OUTPUT.PUT_LINE('2 w_stringa_familiari'||w_stringa_familiari);
         w_dettaglio_ogim := w_dettaglio_ogim||w_dettaglio_pf||w_dettaglio_pv||w_dettaglio_tot;
         if a_oggetto is not null then
            w_dettaglio_ogim := w_dettaglio_ogim||rpad(nvl(w_dettaglio_conf,' '),63);
         end if;
...
-- dbms_output.put_line('Non Trovato');
         ...
         if a_flag_tariffa_base is null then
            ...
            w_dettaglio_pf := ' Coeff. '||lpad(nvl(translate(ltrim(to_char(w_coeff1,'90.0000')),'.,',',.'),' '),7,' ')
                           ||' Tariffa Base ' ||lpad(nvl(translate(ltrim(to_char(w_tari,'999,990.00000')),'.,',',.'),' '),13,' ')
                           ||' Imposta Base QF' ||lpad(nvl(translate(ltrim(to_char(w_importo_pf,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
            w_dettaglio_pv := ' Coeff. '||lpad(nvl(translate(ltrim(to_char(w_coeff2,'90.0000')),'.,',',.'),' '),7,' ')
                           ||' Tariffa Base ' ||lpad(nvl(translate(ltrim(to_char(a_tariffa,'999,990.00000')),'.,',',.'),' '),13,' ')
                           ||' Imposta Base QV' ||lpad(nvl(translate(ltrim(to_char(w_importo_pv,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
         end if;
         --
         -- (VD - 18/11/2016) - Sperimentazione Poasco
         --                     Si verifica se l'utenza ha diritto agli sconti
...
--         ' periodo '||to_char(w_periodo)||' perc_possesso '||to_char(a_perc_possesso)||' importo '||to_char(w_importo));
   ...
   ELSE
      ...
      EXCEPTION
         WHEN others THEN
            w_errore := 'Errore in ricerca Coefficenti Non Domestici per calcolo Normalizzato ['
      ...
      --                ||' Tariffa ' ||lpad(nvl(translate(ltrim(to_char(w_tari,'999,990.00000')),'.,',',.'),' '),13,' ')
      --                ||' Imposta QF' ||lpad(nvl(translate(ltrim(to_char(w_importo_pf,'99,999,999,990.00')),'.,',',.'),' '),17,' ');
      w_importo_pv := (a_tariffa * w_coeff2* b_consistenza)
                   * w_periodo * nvl(a_perc_possesso,100) / 100 * w_coeff_acconto_tares;
      w_importo_pv := round(w_importo_pv,2);
      --
      -- (VD - 30/10/2018): modificata composizione stringa per
   ...
   a_dettaglio_ogim    := substr(w_dettaglio_ogim,1,2000);
EXCEPTION
   WHEN errore THEN
      ROLLBACK;
...
END;
/* End Procedure: CALCOLO_IMPORTO_NORMALIZZATO */
/
