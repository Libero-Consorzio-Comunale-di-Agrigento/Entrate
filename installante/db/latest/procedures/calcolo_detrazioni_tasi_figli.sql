...
--changeset abrandolini:20250326_152423_calcolo_detrazioni_tasi_figli stripComments:false runOnChange:true 
 
create or replace procedure CALCOLO_DETRAZIONI_TASI_FIGLI
/*************************************************************************
  NOME:        CALCOLO_DETRAZIONI_TASI_FIGLI
  DESCRIZIONE: Calcola le detrazioni TASI per figli a carico
  ANNOTAZIONI:
  REVISIONI:
  Rev.  Data        Autore  Descrizione
  ----  ----------  ------  ----------------------------------------------------
  004   13/10/2021  VD      Aggiunto parametro tipo_evento che corrisponde
  ...
  001   14/11/2014  VD      Modificata gestione detrazioni se la detrazione è
                            maggiore dell'imposta per il primo oggetto.
...
--
-- Selezione dei contribuenti su cui e` stata calcolata l`imposta TASI.
...
--
-- Trattamento di Detrazione Figli ACCONTO
--
      if w_tot_detrazione_acc   > 0 then
         ...
         LOOP
            ...
            w_detrazione_acconto_d   := least(nvl(rec_ogim.imposta,0)
...
--
-- Trattamento di Detrazione Figli
--
...
--            w_detrazione_acconto := 0;
--            w_detrazione_acconto_d := 0;
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ROLLBACK;
      RAISE_APPLICATION_ERROR(-20999,'Errore in Calcolo Detrazioni TASI Figli di '||w_cod_fiscale||' ('||SQLERRM||')');
END;
/* End Procedure: CALCOLO_DETRAZIONI_TASI_FIGLI */
/
