...
--changeset abrandolini:20250326_152423_numera_bollettini_ruolo stripComments:false runOnChange:true 
 
create or replace procedure NUMERA_BOLLETTINI_RUOLO
(a_ruolo            in  number
...
nRata_Prec              number;
cursor sel_ogim (p_ruolo       number
...
select raim.rata_imposta
 ...
 order by
       raim.rata asc
;
cursor sel_racf (p_ruolo        number
                ,p_cod_fiscale  varchar2
                ,p_anno         number
...
--
   ...
   nAnno                 := null;
   sTipo_Tributo         := null;
   FOR rec_ogim in sel_ogim(a_ruolo,a_ordinamento,a_cod_fiscale)
   LOOP
...
--
      ...
      and a_tipo_num            = 'CONT' then
         nRata_Prec := -1;
         FOR rec_racf in sel_racf (a_ruolo,sCod_Fiscale_Prec,nAnno,sTipo_Tributo)
         LOOP
...
END;
/* End Procedure: NUMERA_BOLLETTINI_RUOLO */
/
