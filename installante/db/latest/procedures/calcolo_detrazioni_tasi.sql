...
--changeset abrandolini:20250326_152423_calcolo_detrazioni_tasi stripComments:false runOnChange:true 
 
create or replace procedure CALCOLO_DETRAZIONI_TASI
/******************************************************************************
  NOME:        CALCOLO_DETRAZIONI_TASI
  DESCRIZIONE: Calcola le detrazioni TASI per contribuente/anno
  ANNOTAZIONI:
  REVISIONI:
  Rev.  Data        Autore  Descrizione
  ----  ----------  ------  ----------------------------------------------------
  005   13/10/2021  VD      Aggiunto parametro tipo_evento che corrisponde
  ...
  001   04/11/2014  VD      Modificata gestione detrazioni se la detrazione
                            è maggiore dell'imposta per il primo oggetto
...
-- Non vengono estratti i contribuenti con solo oggetti_pratica che
-- hanno una detrazione oggetto (detrazioni_ogco)
...
select ogco.oggetto_pratica        oggetto_pratica
 ...
 where ogco.cod_fiscale               =    ogim.cod_fiscale
   ...
   and ogim.anno                      =    p_anno
   and ogim.ruolo                    is    null
   and (    nvl(ogco.detrazione,0)    > 0 -- SC 22/05/2014 non si sa se potremme mettere ogco.detrazione is not null
        and ogco.flag_ab_principale   =    'S'
        or  nvl(ogco.detrazione,0)    > 0  --SC 22/05/2014 ogco.detrazione is not null
...
is
--
-- (VD - 15/05/2015): Aggiunta decode su wrk_calcolo per trattare prima
-- gli oggetti ai quali sono state applicate le maggiori detrazioni
--
select decode(ogim.detrazione,null,1,
              decode(ogim.wrk_calcolo,'TD',1,2)
             )                     detrazione_ogim
      ,F_DATO_RIOG(ogim.cod_fiscale
                       ,ogim.oggetto_pratica
                       ,ogim.anno
...
--
   FOR rec_cf IN sel_cf (a_cf,a_anno,w_flag_pertinenze,
...
-- una pratica TASI con detrazione per stessi oggetto e contribuente dell`anno
...
--
               update oggetti_imposta
                  set detrazione             = decode(nvl(detrazione,0) + w_detrazione
                                                     ...
                                                     ) /*da capire perchè si vuole questa decode SC 22/05/2014*/
...
-- Nel caso in cui non vi sia la maggiore detrazione per l'anno precedente,
-- vi sia quella per questo anno e la detrazione totale calcolata rispetto
-- ai dati di denuncia sia 0, si prende come detrazione totale per l'acconto
-- la maggiore detrazione dell'anno diviso 2
--
...
--
      ...
      or w_tot_detrazione_prec_d > 0 then
         ...
         LOOP
--w_errore := 'Imposta: '||to_char(rec_ogim.imposta)||' Imposta acconto: '||to_char(rec_ogim.imposta_acconto)||
--            ' Detrazione: '||to_char(w_tot_detrazione) ||' Detrazione acc.: '||to_char(w_tot_detrazione_prec);
--raise errore;
            ...
            update oggetti_imposta
               set detrazione             = decode(nvl(detrazione,0) + w_detrazione_acconto
                  ...
                  ,imposta                = imposta - nvl(w_detrazione_acconto,0)
         ...
         LOOP
            ...
            update oggetti_imposta
               set detrazione             = decode(nvl(detrazione,0) + w_detrazione
                  ...
                  ,imposta                = imposta - nvl(w_detrazione,0)
                  ,imposta_dovuta         = nvl(imposta_dovuta,0) - nvl(w_detrazione_d,0)
                  ,imposta_acconto        = nvl(imposta_acconto,0) - nvl(w_detrazione_acconto,0)
                  ,imposta_dovuta_acconto = nvl(imposta_dovuta_acconto,0) - nvl(w_detrazione_acconto_d,0)
                  ,wrk_calcolo            = 'TD'
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ROLLBACK;
      RAISE_APPLICATION_ERROR(-20999,'Errore in Calcolo Detrazioni TASI di '||w_cod_fiscale||' ('||SQLERRM||')');
END;
/* End Procedure: CALCOLO_DETRAZIONI_TASI */
/
