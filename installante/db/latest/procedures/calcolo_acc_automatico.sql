...
--changeset abrandolini:20250326_152423_calcolo_acc_automatico stripComments:false runOnChange:true 
 
CREATE OR REPLACE procedure CALCOLO_ACC_AUTOMATICO
/*************************************************************************
  Rev.    Date         Author      Note
  014     20/05/2025   RV          #77609
                                   Adeguamento nuovo DL regime sanzionatorio
  013     15/04/2024   RV          #70776
                                   Aggiunto gestione separata per pratiche annuali e temporanee
                                   Sistemato contabilizzazione dei versamenti in accordo
  012     09/04/2024   RV          #54732
                                   Aggiunto gestione date personalizzate e generazione
                                   distinta di un accertamento per gruppo tributo
  011     19/02/2024   RV          #69834
                                   Esclusione oggetti CUNI nati il 31/12/anno_calcolo
  010     20/09/2022   VM          #66699 - sostituito filtro ricerca sogg.cognome_nome
                                   con sogg.cognome_nome_ric
  009     08/03/2023   VM          Issue #61132
                                   Aggiunto parametro a_se_spese_notifica
  008     30/01/2023   AB          Tolto il controllo di sol_prec x utente #
  007     23/01/2023   RV          Issue #61793
                                   Rivisto parametri e filtri per
                                   tipo solleciti.
  006     17/01/2023   RV          Issue #61561
                                   Aggiunta parametri e filtri per
                                   solo sollecitati.
  5       22/03/2022   VD          Aggiunta selezione versamenti su
                                   ravvedimento.
  4       14/03/2022   VD          Verificato per gestione CUNI.
                                   Modificato test su codici sanzioni (ora
                                   si usa il tipo causale).
                                   Aggiunta selezione flag_canone per CUNI.
  3       08/04/2021   VD          Modificata selezione versamenti da
                                   scalare: si trattano solo i versamenti
                                   dell'anno con pratica null oppure relativi
                                   a denunce dell'anno per occupazione
                                   permanente
  2       13/03/2019   VD          Selezione pratiche da eliminare:
                                   aggiunto nvl a zero dell'importo totale
                                   per eliminare anche pratiche prive di
                                   sanzioni
...
,a_tipo_solleciti        in varchar2 default null   /* 'T' (Def) - Tutti, 'S' - Solo Sollecitati, 'M' - Solo NON Sollecitati */
,a_data_sollecito_da     in date default null
,a_data_sollecito_a      in date default null
,a_data_notifica_sol_da  in date default null
,a_data_notifica_sol_a   in date default null
) IS
--
...
w_oggetto               number;
w_pratica_da_acc_prec   number;
w_pratica_da_acc        number;
w_pratica               number;
w_data_pratica          date;
...
--
CURSOR sel_liq ( p_tipo_trib    varchar2
               , p_anno         number
...
-- RV (19/02/2024) : #69834 Solo per CUNI escludere sempre dal calcolo gli oggetti nati il 31/12/anno_calcolo
   ...
   and ((nvl(a_tipo_solleciti,'T') = 'T')
        or
        ((nvl(a_tipo_solleciti,'T') = 'S')
          and exists
          (select 'x'
           ...
           where prt3.tipo_tributo = p_tipo_trib
             ...
             and prt3.tipo_pratica = 'S'
             and nvl(prt3.stato_accertamento,'D') = 'D'
             and prt3.data >= nvl(a_data_sollecito_da,to_date('01011900','ddmmyyyy'))
             and prt3.data <= nvl(a_data_sollecito_a,to_date('31122999','ddmmyyyy'))
             and nvl(prt3.data_notifica,to_date('01011990','ddmmyyyy')) >=
                 nvl(a_data_notifica_sol_da,to_date('01011900','ddmmyyyy'))
             and nvl(prt3.data_notifica,to_date('01011990','ddmmyyyy')) <=
                 nvl(a_data_notifica_sol_a,to_date('31122999','ddmmyyyy')))
         )
        or
        ((nvl(a_tipo_solleciti,'T') = 'N')
          and not exists
          (select 'x'
            from pratiche_tributo prt3
           where prt3.tipo_tributo = p_tipo_trib
...
select decode(p_scadenza_r1
 ...
 where vers.cod_fiscale                      = p_cod_fiscale
   ...
   --                    - senza pratica per il calcolo anni precedenti (p_pratica_da_acc = null)
   --                    - solo quelli collegati alla pratica per gli altri casi
   and (
        ((p_tipo_tributo = 'CUNI') and
         (((p_pratica_da_acc is null) and (vers.pratica is null))
         or
         (vers.pratica = p_pratica_da_acc)
         )
        )
       or
        ((p_tipo_tributo != 'CUNI') and
...
BEGIN
   ...
   loop
      begin
       --dbms_output.put_line('Elimino accertamento precedente: '||rec_liq.pratica);
   ...
   if sel_acc%FOUND then
    --dbms_output.put_line('*** Accertamento per: '||w_cod_fiscale||', gruppo: '||nvl(w_gruppo_tributo,'NULL')||', pratica: '||w_pratica_da_acc);
      w_cod_fiscale_prec   := w_cod_fiscale;
      w_gruppo_tributo_prec := w_gruppo_tributo;
      w_pratica_da_acc_prec := w_pratica_da_acc;
      w_imposta_r1         := 0;
      w_imposta_r2         := 0;
      w_imposta_r3         := 0;
      w_imposta_r4         := 0;
...
--
         ...
         EXCEPTION
            WHEN NO_DATA_FOUND THEN
               ...
               w_scad         := null;
         END;
         -- Gestione Arrotondamenti pratiche dell'anno di accertamento
         ...
         --
       --dbms_output.put_line('C.F.: '||w_cod_fiscale||', gruppo: '||nvl(w_gruppo_tributo,'NULL')||', pratica: '||w_pratica_da_acc);
    ...
    --   w_imposta_rt         := w_imposta_rt + w_dep_rt;
         ...
         or nvl(w_pratica_da_acc,0) <> nvl(w_pratica_da_acc_prec,0) then
          --dbms_output.put_line('*** Genera pratica: '||w_cod_fiscale_prec||', gruppo: '||nvl(w_gruppo_tributo_prec,'NULL')||', pratica: '||w_pratica_da_acc_prec);
            w_cf := w_cod_fiscale_prec;
            w_gt := w_gruppo_tributo_prec;
            w_pr := w_pratica_da_acc_prec;
            --
...
--                and (raim.oggetto_imposta is null)
                  and (
                       ((w_pratica_da_acc_prec is null) and (raim.oggetto_imposta is null)
                       )
                      or
                       ((w_pratica_da_acc_prec is not null) and
                        (raim.oggetto_imposta in
                          (select ogim.oggetto_imposta
                            ...
                            where ogim.oggetto_pratica = ogpr.oggetto_pratica
                              and ogim.anno = a_anno
                              and ogpr.pratica = w_pratica_da_acc_prec)
                        )
                       )
                  )
                ;
...
--dbms_output.put_line('ACCERTAMENTO - Anno '||a_anno);
...
--dbms_output.put_line('Stringa Eccedenze '||w_stringa_eccedenze_r4);
         ...
         if sel_acc%NOTFOUND then
            exit;
                ...
                nvl(w_pratica_da_acc_prec,0) <> nvl(w_pratica_da_acc,0)) then
          --dbms_output.put_line('*** Accertamento per: '||w_cod_fiscale||', gruppo: '||nvl(w_gruppo_tributo,'NULL')||', pratica: '||w_pratica_da_acc);
            w_cod_fiscale_prec := w_cod_fiscale;
            w_gruppo_tributo_prec := w_gruppo_tributo;
            w_pratica_da_acc_prec := w_pratica_da_acc;
   ...
   close sel_acc;
--
-- Per TOSAP si attiva la gestione delle Pratiche dell`anno con data concessione.
--
   if a_tipo_tributo = 'TOSAP' then
      CALCOLO_ACC_CONCESSIONI(a_tipo_tributo
                            , a_anno
                            , a_cod_fiscale
                            , a_cognome_nome
                            , a_utente
...
EXCEPTION
   ...
   WHEN others THEN
      ...
      (-20999,'Errore in Calcolo Automatico Accertamenti di '||w_cf||' ('||SQLERRM||')');
END;
/* End Procedure: CALCOLO_ACC_AUTOMATICO */
/
