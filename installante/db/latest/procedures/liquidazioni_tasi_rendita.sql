...
--changeset abrandolini:20250326_152423_liquidazioni_tasi_rendita stripComments:false runOnChange:true 
 
create or replace procedure LIQUIDAZIONI_TASI_RENDITA
/*************************************************************************
 Versione  Data              Autore    Descrizione
 15        14/04/2025        RV        #77608
                                       Adeguamento gestione sequenza sanzioni 
 14        30/05/2024        AB        #73009 Acquisito anche imm_storico di ogpr e inserito nella Liq
 13        08/04/2024        AB        #71613 Utilizzato il campo w_mese_inizio in relazione ai mesi
                                       di inizio periodo per individuare un unico record e cosi
                                       avere la rendita e il valore corretti, altrimenti null
 12        25/09/2023        RV        #66351 Accetta valore_dic null per tipi 1 e tipi 3 con Ctg. E ed F
 11        22/08/2023        AB        #66402 Aggiunti controlli per valorizzare correttamente
                                       mese inizio e fine, nel caso di da_mese > 12
 10        09/08/2023        AB        Aggiunti controlli per valorizzare correttamente
                                       mese inizio e fine
 9         02/08/2023        VM        #65986 - Fix calcolo mesi esclusione in base al flag esclusione
 8         08/05/2023        AB        Controllo periodi considerando data_fine >= w_anno
 7         31/01/2023        AB        Controllo date utilizzando anche periodi_riog
 6         21/12/2022        AB        Nel caso di importo a 0 e mesi esclusione
                                       presenti o flag_esclsuione = 'S'
                                       prendo il valore da valore_dic rivalutato
 5         15/12/2022        AB        Gestione del da_mese_possesso per il
                                       controllo dei riog e annullo rev 4
 4         02/08/2022        VD        Aggiunto test su flag esclusione per
                                       non trattare gli immobili esclusi
 3         02/12/2015        VD        Modificata gestione fabbricati "D":
                                       ora solo totalizzati insieme agli
                                       "altri fabbricati".
                                       Eliminata gestione importi divisi
                                       per comune/erario
 2         25/03/2015        VD        Aggiunta selezione e inserimento
                                       in OGGETTI_IMPOSTA dei campi
                                       mesi_possesso, mesi_affitto e
                                       percentuale
 1         24/03/2015        VD        Aggiunto inserimento tipo_rapporto
                                       in OGGETTI_IMPOSTA
*************************************************************************/
(a_anno                                  number,
 a_pratica                               number,
...
CURSOR sel_ogim (p_anno number, p_cod_fiscale varchar2) IS
       select ogpr.oggetto,ogpr.tipo_oggetto
            ...
            , f_valore(nvl(f_valore_d(ogpr.oggetto_pratica,p_anno),ogpr.valore)
             ...
             , ogco.tipo_rapporto
             , ogco.perc_possesso
             , decode(ogco.anno,p_anno,nvl(ogco.mesi_possesso,12),12)           mesi_possesso
             ...
             , decode(ogco.anno
                     ,p_anno,decode(ogco.flag_riduzione
            ...
            , ogim.mesi_possesso mesi_possesso_ogim
            , ogim.mesi_affitto
            , ogim.percentuale
...
--------------------------------------------------------------------------------
-- LIQUIDAZIONI_TASI_RENDITA
--------------------------------------------------------------------------------
BEGIN
--dbms_output.put_line('Liquidazione TASI rendita');
   wf := 0;
   --
...
--dbms_output.put_line('Rendita - Oggetto pratica: '||rec_ogim.oggetto_pratica);
      ...
      ELSE
         w_bError := FALSE;
      END IF;
--dbms_output.put_line('Errore: '||rec_ogim.oggetto_pratica||' - '||w_mError);
      IF w_bError THEN
         wf := 1;
         BEGIN
      ...
      ELSE
         ...
         IF rec_ogim.tipo_pratica = 'A' and rec_ogim.anno = a_anno THEN
            ...
            -- Altri fabbricati (comprensivi di fabbricati D)
...
-- AB (21/12/2022) Aggiunto per avere il valore e la rendita nella liquidazione
            ...
            BEGIN
              insert into oggetti_pratica
                     ...
                     , tipo_oggetto
                     , imm_storico)
              select   w_oggetto_pratica
                     , rec_ogim.oggetto
                     , a_pratica
                     , a_anno
                     , rec_ogim.categoria_catasto
                     , rec_ogim.classe_catasto
                     , w_valore_ogpr
                     , rec_ogim.oggetto_pratica
                     , a_utente
                     , trunc(sysdate)
                     , rec_ogim.tipo_oggetto
            ...
            BEGIN
              wf := 5;
--
-- VD 24/03/2015: Si inserisce il tipo_rapporto solo su OGGETTI_IMPOSTA
--                per visualizzarlo nella window di liquidazione TASI
--
              insert into oggetti_imposta
                     (oggetto_imposta,cod_fiscale,anno,
                      ...
                      mesi_possesso,mesi_affitto,percentuale)
              values (w_oggetto_imposta,a_cod_fiscale,a_anno,
                      ...
                      a_utente,trunc(sysdate),
                      'TASI',rec_ogim.tipo_rapporto,
                      rec_ogim.mesi_possesso_ogim,rec_ogim.mesi_affitto,rec_ogim.percentuale)
              ;
            EXCEPTION
              WHEN others THEN
            ...
            IF nvl(rec_ogim.valore,0) > nvl(rec_ogim.valore_dic,0)
              ...
              THEN
               ...
               IF nvl(w_imponibile_10_acconto,0) > 0 THEN  -- INIZIO CONTROLLO ANOMALIE 110
                  ...
                  ELSIF w_check = 1 THEN
                    aggiornamento_sanz_liq_tasi(w_cod_sanzione,C_TIPO_TRIBUTO
                  ...
                  END IF;
               END IF; -- FINE CONTROLLO ANOMALIE 110
               IF nvl(w_imponibile_10_saldo,0) > 0 THEN -- INIZIO CONTROLLO ANOMALIE 120
                  ...
                  ELSE
                    w_errore := 'Errore f_check_sanzione per sanzione: '||w_cod_sanzione;
                  END IF;
               END IF; -- FINE CONTROLLO ANOMALIE 120
            END IF; -- FINE CONTROLLO ANOMALIE  110 , 120
...
EXCEPTION
  ...
  WHEN others THEN
       ROLLBACK;
       RAISE_APPLICATION_ERROR (-20999,'Errore in LIQUIDAZIONI TASI RENDITA -'||to_char(wf)||'- ('||SQLERRM||')');
END;
/* End Procedure: LIQUIDAZIONI_TASI_RENDITA */
/
