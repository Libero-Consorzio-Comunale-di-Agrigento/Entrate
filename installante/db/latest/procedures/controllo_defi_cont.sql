...
--changeset abrandolini:20250326_152423_controllo_defi_cont stripComments:false runOnChange:true 
 
create or replace procedure CONTROLLO_DEFI_CONT
...
cursor sel_figl (p_cod_fam         number
   ...
   select sogg.matricola
    ...
    where sogg.cod_fam         = p_cod_fam
      ...
      and sogg.data_ult_eve  < a_data_rif
      and Floor(Months_Between(a_data_rif,sogg.data_nas)/12)
                               < nvl(to_number(F_INPA_VALORE('IMU_ETA_FG')),26)
      and Floor(Months_Between(a_data_rif,sogg.data_nas)/12) >= 0
   ...
   select sogg.matricola
    ...
    where sogg.cod_fam         = p_cod_fam
      ...
      and sogg.matricola      <> p_matricola
       ;
cursor sel_ctr (p_cod_fiscale     varchar2
               ,p_anno            number
               ,p_data_rif        date
                ) is
  select cod_fiscale
    from ( (select defi1.cod_fiscale
                 , defi1.anno
                 , defi1.da_mese
                 , defi1.a_mese
                 , defi1.numero_figli
                 , defi1.detrazione
                 , defi1.detrazione_acconto
              from detrazioni_figli  defi1
             where defi1.anno             = p_anno
               and defi1.cod_fiscale   like p_cod_fiscale
               and defi1.da_mese           = 1
            minus
            select defc1.cod_fiscale
                 , defc1.anno
                 , 1                      da_mese
                 , 12                     a_mese
                 , defc1.numero_figli
                 , defc1.detrazione
                 , defc1.detrazione_acconto
              from detrazioni_figli_ctr  defc1
             where defc1.anno             = p_anno
               and defc1.data_riferimento = p_data_rif
               and defc1.cod_fiscale   like p_cod_fiscale
           )
          union
           (select defc2.cod_fiscale
                 , defc2.anno
                 , 1                      da_mese
                 , 12                     a_mese
                 , defc2.numero_figli
                 , defc2.detrazione
                 , defc2.detrazione_acconto
              from detrazioni_figli_ctr  defc2
             where defc2.anno             = p_anno
               and defc2.data_riferimento = p_data_rif
               and defc2.cod_fiscale   like p_cod_fiscale
            minus
            select defi2.cod_fiscale
                 , defi2.anno
                 , defi2.da_mese
                 , defi2.a_mese
                 , defi2.numero_figli
                 , defi2.detrazione
                 , defi2.detrazione_acconto
              from detrazioni_figli  defi2
             where defi2.anno             = p_anno
               and defi2.cod_fiscale   like p_cod_fiscale
            )
          )
...
BEGIN
   ...
   -- Cancellazione Tabella Detrazioni Figlio di Controllo
   begin
      delete detrazioni_figli_ctr  defi
       where defi.anno             = a_anno
         and defi.data_riferimento = a_data_rif
         and defi.cod_fiscale   like a_cod_fiscale
           ;
   EXCEPTION
      WHEN OTHERS THEN
      raise_application_error(-20999,'Errore Cancellazione Tabella Detrazioni Figlio di Controllo '||
                                     'anno '||to_char(a_anno)||'data_rif '||to_char(a_data_rif,'dd/mm/yyyy')||
                                     'cf '||a_cod_fiscale);
   end;
   FOR rec_ogco in sel_ogco
   LOOP
      if ( rec_ogco.fascia  in (1,3) and rec_ogco.data_ult_eve  < a_data_rif )
        or
         ( rec_ogco.fascia  in (2,4) and rec_ogco.data_ult_eve >= a_data_rif )
        then
         ...
         begin
            select count(1)
              into nContaDefi
              from detrazioni_figli_ctr  defi
            ...
            if rec_ogco.tipo_oggetto = 3 and
               ...
               if nNumeroFigli > 0 then
                  BEGIN
                     ...
                     insert into detrazioni_figli_ctr
                           ( cod_fiscale
                           , anno
                           , data_riferimento
                           , numero_figli
  ...
  begin
    delete wrk_generale
     where upper(tipo_trattamento) = 'DEFI_CONT'
         ;
  EXCEPTION
     WHEN others THEN
        RAISE_APPLICATION_ERROR(-20999,'Errore in Eliminazione WRK_GENERALE ');
  end;
   FOR rec_ctr in sel_ctr(a_cod_fiscale, a_anno, a_data_rif)
   LOOP
      nTrattati := nTrattati + 1;
        BEGIN
            ...
            values ('DEFI_CONT'
                   ,a_anno
                   ,to_number(to_char(sysdate,'mmddhh24mi'))*1000000 + nTrattati
                   ,rec_ctr.cod_fiscale)
            ;
        EXCEPTION
            WHEN others THEN
               RAISE_APPLICATION_ERROR(-20999,'Errore in inserimento wrk_generale ('||SQLERRM||')');
        END;
   end loop;
   if nTrattati = 0 then
      sMessagggio := 'Nessuna Anomalia';
   else
      sMessagggio := 'Esistono '||to_char(nTrattati)||' contribuenti con Anomalie.';
   end if;
...
END;
/* End Procedure: CONTROLLO_DEFI_CONT */
/
