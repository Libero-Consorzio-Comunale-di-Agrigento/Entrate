...
--changeset abrandolini:20250326_152423_mod_min_denuncia_ici stripComments:false runOnChange:true 
 
create or replace procedure MOD_MIN_DENUNCIA_ICI
(a_sessione          in number
,a_pratica           in number
) is
--
-- Pre-Trattamento dei dati per la stampa del modello
-- ministeriale di denuncia ICI.
--
...
-- e/o 3 contitolari o per il numero dei rimanenti a fine analisi
-- dei dati, si inserisce in parametri una registrazione con:
-- sessione da parametri di procedura, nome parametro = 'MODMINDENICI'
-- che sta per Modello Ministeriale Denuncia ICI, progressivo di 1 in 1
   ...
   BEGIN
      delete from parametri
       where sessione       = a_sessione
         and nome_parametro = 'MODMINDENICI'
      ;
   END;
   commit;
--
-- Si contano i moduli necessari per la denuncia
-- (1 ogni 3 contitolari e/o 3 oggetti).
--
   nProgressivo     := 0;
--
...
-- a 3 a 3 e solo al terzo contitolare viene
   ...
   LOOP
      ...
      if nConta = 3 then
         ...
         BEGIN
            ...
            values(a_sessione,'MODMINDENICI',nProgressivo,sStringa2);
         END;
         sStringa2 := lpad(to_char(a_pratica),10,'0')||
         ...
         BEGIN
            select para.valore
             ...
             where para.sessione         = a_sessione
               and para.nome_parametro   = 'MODMINDENICI'
...
EXCEPTION
   WHEN FINE THEN
      commit;
END;
/* End Procedure: MOD_MIN_DENUNCIA_ICI */
/
