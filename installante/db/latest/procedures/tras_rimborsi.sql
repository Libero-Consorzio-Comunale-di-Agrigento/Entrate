...
--changeset abrandolini:20250326_152423_tras_rimborsi stripComments:false runOnChange:true 
 
CREATE OR REPLACE procedure     TRAS_RIMBORSI
/*************************************************************************
 NOME:        TRAS_RIMBORSI

 DESCRIZIONE: Estrazione dati rimborsi ICI

 Data        Autore      Descrizione
 24/11/2021  VD          Corretta formattazione importo totale imposte
                         in LIRE
 XX/XX/XXXX  XX          Prima emissione.
**************************************************************************/
...
w_coda              varchar2(20);
w_estremi_sentenza  varchar2(16)   := lpad(' ',16);
w_data_sentenza     varchar2(8)    := lpad('0',8,'0');
w_estremi           varchar2(16);
...
select rpad(nvl(sogg.cognome_nome,' '),60)            denominazione
      ,rpad(nvl(sogg.cognome,' '),24)                 cognome
      ,rpad(nvl(substr(sogg.nome,1,20),' '),20)       nome
...
select nvl(sum(decode(sanz.flag_interessi
      ...
      ,nvl(sum(decode(sanz.flag_interessi
                     ,'S',0
      ...
      ,nvl(sum(decode(sanz.flag_interessi
                     ,'S',nvl(sapr.importo,0)
                         ,0
                     )
              ),0
          ) interessi
      ,nvl(sum(decode(sanz.flag_interessi
                     ,'S',round(nvl(sapr.importo,0)
                                * (100 - nvl(sapr.riduzione,0)) / 100,2
                               )
                         ,0
                     )
...
BEGIN  -- TRASMISSIONE_RIMBORSI
  ...
  END;
-- w_progressivo parte da MIN_REC perche`
-- i primi due record (tipo 0 e 1) vengono inseriti in fondo
  ...
  FOR rec_prtr IN sel_dati_prtr(a_anno) LOOP
     BEGIN
        -- Totale dei versamenti relativi ad Accertamenti e Liquidazioni
        -- del Contribuente
     ...
     EXCEPTION
       WHEN no_data_found THEN
           ...
           w_data_versamento := 0;
       WHEN others THEN
           w_errore := 'Errore nel recupero del totale versato del contribuente '||
                       rec_prtr.cod_fiscale||' ('||SQLERRM||')';
           RAISE errore;
     END;
     IF w_versato < 0 THEN
        FOR rec_cont in sel_dati_cont(rec_prtr.cod_fiscale) LOOP
           ...
           ELSE
              ...
              w_dati_cont := '2'||rec_cont.denominazione||rec_cont.des_res
                                ||rec_cont.sigla_pro_res||rpad(rec_prtr.cod_fiscale,16,' ');
              w_flag_tipo := 2;
              w_estremi   := lpad(' ',16);
              w_coda      := lpad(' ',7);
           END IF;
        ...
        w_riga := w_dati_cont||w_estremi||lpad(nvl(w_data_versamento,0),8,'0')||
                  w_estremi_sentenza||w_data_sentenza;
        if w_fase_euro = 1 then
           w_riga := w_riga||lpad(abs(trunc(w_imposta)),10,0)||
                     lpad(abs(trunc(w_interessi)),10,0)||'0'||w_coda;
        else
           w_riga := w_riga||lpad(abs(w_imposta) * 100,10,0)||
                     lpad(abs(w_interessi) * 100,10,0)||'1'||w_coda;
        end if;
        w_tot_imposte   := w_tot_imposte   + abs(w_imposta);
        w_tot_interessi := w_tot_interessi+ abs(w_interessi);
        w_imposta    := 0;
        w_interessi := 0;
        BEGIN
  ...
  IF w_progressivo = MIN_REC THEN
     w_errore := 'Non e'' presente nessun rimborso. Impossibile creare il file relativo.';
     RAISE errore;
  ELSE
     -- CREAZIONE TIPO RECORD 0
     w_appoggio := w_sigla_cf||'R'||'001';
     w_aliquota_anno := f_aliquota_base(a_anno);
     if w_aliquota_anno = -1 then
        w_errore := 'Non è presente l`aliquota base per l`anno '||to_char(a_anno);
        RAISE ERRORE;
     end if;
     w_riga := '0'||w_appoggio||
               to_char(sysdate,'yyyy')||to_char(sysdate,'mm')||to_char(sysdate,'dd')||
               w_des_comune||w_sigla_provincia||
               lpad(to_char(w_aliquota_anno),5,'0')||lpad(' ',129);
     BEGIN
        insert into WRK_TRAS_ANCI (anno, progressivo,dati)
     ...
     w_riga := '9'||w_appoggio||to_char(sysdate,'yyyy')||
               to_char(sysdate,'mm')||to_char(sysdate,'dd')
               ||lpad(w_num_soggetti,6,0)||lpad(w_num_societa,5,0);
     if w_fase_euro = 1 then
        w_riga := w_riga
                  ||lpad(to_char(w_tot_imposte),16,'0')||lpad(to_char(w_tot_interessi),16,'0')
                  ||lpad(to_char(round(w_tot_imposte/w_euro,2)*100),16,'0')
                  ||lpad(to_char(round(w_tot_interessi/w_euro,2)*100),16,'0');
     else
        w_riga := w_riga
                  ||lpad(to_char(round(w_tot_imposte*w_euro,0)),16,'0')
                  ||lpad(to_char(round(w_tot_interessi*w_euro,0)),16,'0')
                  --||lpad(to_char(w_tot_imposte*100,16),'0')
                  ||lpad(to_char(w_tot_imposte*100),16,'0')
                  ||lpad(to_char(w_tot_interessi*100),16,'0');
     end if;
     w_riga := w_riga||lpad(' ',88);
     BEGIN
        insert into WRK_TRAS_ANCI (anno,progressivo,dati)
...
EXCEPTION
   ...
   RAISE_APPLICATION_ERROR
          (-20999,'Errore in Trasmissione Rimborsi su supporto magnetico ('||SQLERRM||')');
END;
/* End Procedure: TRAS_RIMBORSI */
/
