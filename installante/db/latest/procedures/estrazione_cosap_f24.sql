...
--changeset abrandolini:20250326_152423_estrazione_cosap_f24 stripComments:false runOnChange:true 
 
create or replace procedure ESTRAZIONE_COSAP_F24
/*************************************************************************
 NOME:        ESTRAZIONE_COSAP_F24
 DESCRIZIONE: Estrazione dati COSAP comprensivi di F24 per invio massivo
              comunicazioni
 NOTE:        Personalizzazioni:
              048033 - Pontassieve
              048010 - Castelfiorentino (tratta solo il codice tributo 453)
 Rev.    Date         Author      Note
 001     03/10/2018   VD          Corrette condizioni di where su tributo
                                  nella query relativa agli oggetti imposta
                                  del contribuente.
...
select ogge.oggetto
     , decode(ogpr.num_concessione
             ,null,''
             ,'Concessione n.'||to_char(ogpr.num_concessione)
              ||decode(ogpr.data_concessione
                      ,null,''
                      ,' del '||to_char(ogpr.data_concessione,'dd/mm/yyyy')
                      )
             )                                                                    concessione
     , decode(ogge.descrizione,'','','Descrizione: '||ogge.descrizione) descrizione
     , 'Ubicazione occupazione: '
       ||decode(w_flag_provincia,'S',decode(ad4_comuni.denominazione,null,''
     ...
     , rpad(decode(cate.descrizione,null,' ',substr('Categoria: '||cate.descrizione,1,70)),70)
       ||lpad('Sup/Nr: '||ogpr.consistenza,17)     desc_categoria
     , decode(ogco.perc_possesso,'','','Perc.'||translate(to_char(ogco.perc_possesso,'990.00'),',.','.,'))     esenzione
...
BEGIN
   ...
   BEGIN
      select lpad(to_char(dage.pro_cliente), 3, '0') ||
           ...
           , comu.denominazione
           , decode(dage.flag_provincia,'S',prov.sigla,comu.sigla_cfis)
           , decode(dage.flag_provincia,'S','PROVINCIA DI ','COMUNE DI ')
           , nvl(flag_provincia,'N')
        into w_istat
           , w_comune
           , w_cod_belfiore
           , w_intest_luogo
       ...
       where dage.pro_cliente = comu.provincia_stato
         ...
         and dage.pro_cliente = prov.provincia
           ;
   EXCEPTION
      WHEN others THEN
   ...
   LOOP
      ...
      LOOP
        if rec_ogim.concessione is not null then
           w_conta_utenze := w_conta_utenze + 1;
           w_riga_var := w_riga_var||rec_ogim.concessione||';';
        end if;
        --
      ...
      w_riga_wrk := w_riga_wrk||w_intest_luogo||w_comune||' - UFFICIO TRIBUTI'||';';
      --
...
END;
/* End Procedure: ESTRAZIONE_COSAP_F24 */
/
