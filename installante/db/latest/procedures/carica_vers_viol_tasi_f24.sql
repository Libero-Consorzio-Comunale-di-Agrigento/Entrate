...
--changeset abrandolini:20250326_152423_carica_vers_viol_tasi_f24 stripComments:false runOnChange:true 
 
create or replace procedure CARICA_VERS_VIOL_TASI_F24
/*******************************************************************************
 Versione  Data        Autore    Descrizione
 013       16/03/2023  VM        #55165 - Aggiunto parametro a_cod_fiscale.
                                 Aggiunto filtro cod_fiscale su popolamento cursori.
 012       14/03/2023  VM        #60197 - Aggiunto il parametro a_log_documento.
 ...
 11        11/01/2021  VD        Gestione nuovo campo note_versamento
                                 ...
                                 VERSAMENTI.
 10        26/10/2018  VD        Gestione importi sanzioni e interessi
 ...
 7         15/06/2016  VD        Aggiunto controllo tipo record in query
                                 principale: si trattano solo i record
                                 G1 - versamenti
 6         04/12/2015  VD        Aggiunta upper in selezione id_operazione
 5         10/03/2015  VD        Corretto test su codici tributo in eliminazione
                                 WRK_TRAS_ANCI
 ...
 3         12/12/2014  VD        Aggiunta gestione nuovo campo
                                 ...
                                 In presenza di identificativo operazione il
                                 versamento viene caricato nella tabella versamenti
...
-- (VD - 17/05/2017): Aggiunto raggruppamento per data versamento
--
CURSOR sel_vers IS
select max(cont.cod_fiscale)                             cod_fiscale
      ...
      ,sum(decode(substr(wkta.dati,79,4)
                 ,'3963',(to_number(substr(wkta.dati,96,15)) - to_number(substr(wkta.dati,111,15))) / 100
                 ,'378E',(to_number(substr(wkta.dati,96,15)) - to_number(substr(wkta.dati,111,15))) / 100
                 ,0
                 )
          )                                               importo_sanzioni
      ,sum(decode(substr(wkta.dati,79,4)
                 ,'3962',(to_number(substr(wkta.dati,96,15)) - to_number(substr(wkta.dati,111,15))) / 100
                 ,'377E',(to_number(substr(wkta.dati,96,15)) - to_number(substr(wkta.dati,111,15))) / 100
                 ,0
                 )
          )                                               importo_interessi
      ,0                                                  ab_principale
 ...
 where cont.cod_fiscale     (+) = rtrim(substr(wkta.dati,50,16))
   and sogg.ni              (+) = cont.ni
   and wkta.anno                = 2
   and substr(wkta.dati,79,4)in ('3962','3963','377E','378E')  -- solo i versamenti su violazioni
 ...
 group by rtrim(substr(wkta.dati,50,16))                       -- cod. fiscale
         ...
         ,to_date(substr(wkta.dati,67,8),'yyyymmdd')           -- data versamento
         ,substr(wkta.dati,84,4)                               -- rateazione
...
-----------------------------------------------------------
   ...
   LOOP
      if rec_errati.cod_fiscale_cont is null then
         ...
         -- (VD - 25/09/2018): aggiunta gestione pratiche rateizzate
         --
...
------------------------------------------------------------
   ...
   LOOP
      if rec_vers.cod_fiscale is null then
         ...
         values(w_progressivo,w_tipo_tributo,'F24',rec_vers.anno,null,rec_vers.cod_fiscale_vers,
                ...
                'Sanzioni e Interessi - Contribuente ('||rec_vers.cod_fiscale_vers||') sconosciuto',
         ...
         if nvl(w_pratica,-1) < 0 then
            ...
            -- da quello della pratica
            -- (VD - 25/09/2018): gestione pratiche rateizzate.
            --                    Si estrae anche la rata dell'identificativo
            --                    operazione
            --
   ...
   -- dbms_output.put_line('ins wrk2  '||w_progressivo||' '||SQLERRM);
               ...
               values(w_progressivo,w_tipo_tributo,'F24',rec_vers.anno,null,rec_vers.cod_fiscale_vers,
                      ...
                      'Sanzioni e Interessi'||decode(w_rata_pratica,0,' - Versamento gia` Presente in data '||to_char(rec_vers.data_pagamento,'dd/mm/yyyy')
                                                                     ,'Pratica rateizzata: Rata gia'' versata'),
                      rec_vers.tipo_versamento,rec_vers.ufficio_pt,rec_vers.data_pagamento,
...
END;
/* End Procedure: CARICA_VERS_VIOL_TASI_F24 */
/
