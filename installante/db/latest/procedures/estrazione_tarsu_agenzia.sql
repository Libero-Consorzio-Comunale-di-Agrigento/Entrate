...
--changeset abrandolini:20250326_152423_estrazione_tarsu_agenzia stripComments:false runOnChange:true 
 
create or replace procedure ESTRAZIONE_TARSU_AGENZIA
(
  p_da_data in date
, p_a_data  in date
)
...
--prtr.data compresa fra da_data e a_data (denunce o accertamenti sostitutivi di denuncia)
--oppure
--ogpr.data_anagrafe_tributaria;
--ogpr.titolo_occupazione, ogpr.natura_occupazione e ogpr.destinazione_uso valorizzati;
--se anno della data di riferimento (quindi prtr.data o ogpr.data_anagrafe_tributaria) nn è 2007 o 2008
--almeno uno fra ogge.foglio, ogge.numero, ogge.subalterno deve essere valorizzato.
cursor sel_tarsu_sog(w_da_data date, w_a_data date) is
   select distinct
          prtr.cod_fiscale                     cod_fiscale
        --Persona fisica
        ...
        , substr(to_char(prtr.data,'dd/mm/yyyy'),7)             anno_riferimento
    ...
    where comf.provincia_stato    = prof.provincia (+)
      ...
      and (prtr.tipo_pratica      = 'D'
       or (prtr.tipo_tributo      = 'A' and prtr.flag_denuncia is not null and prtr.data_notifica is not null and nvl(prtr.stato_accertamento,'D') = 'D'))
      and (substr(to_char(prtr.data,'dd/mm/yyyy'),7)
                                        not in ('2007','2008')
       and (ogge.foglio                            is not null
         or ogge.numero                            is not null
         or ogge.subalterno                        is not null)
       or (substr(to_char(prtr.data,'dd/mm/yyyy'),7))
                                        in ('2007','2008'))
      and ogpr.titolo_occupazione is not null
      and ogpr.natura_occupazione is not null
      and ogpr.destinazione_uso   is not null
      and prtr.data               between w_da_data --to_date(w_da_data, 'dd/mm/yyyy')
                                      and w_a_data --to_date(w_a_data, 'dd/mm/yyyy')
UNION
select distinct
        ...
        , decode(sogg.tipo
                         , 1, nvl(prog.sigla,' ')
                         , ' ')                 sigla_prov_sede_legale
        , substr(to_char(ogpr.data_anagrafe_tributaria,'dd/mm/yyyy'),7)             anno_riferimento
     from ad4_comuni       comf
        , ad4_provincie    prof
        , ad4_comuni       comg
        , ad4_provincie    prog
    ...
    where comf.provincia_stato          = prof.provincia (+)
      ...
      and cont.ni                       = sogg.ni
      and ogpr.pratica                  = prtr.pratica
      and (substr(to_char(ogpr.data_anagrafe_tributaria,'dd/mm/yyyy'),7)
                                        not in ('2007','2008')
       and (ogge.foglio                            is not null
         or ogge.numero                            is not null
         or ogge.subalterno                        is not null)
       or (substr(to_char(ogpr.data_anagrafe_tributaria,'dd/mm/yyyy'),7))
                                        in ('2007','2008'))
      and ogpr.titolo_occupazione       is not null
      and ogpr.natura_occupazione       is not null
      and ogpr.destinazione_uso         is not null
      and ogpr.data_anagrafe_tributaria between w_da_data --to_date(w_da_data, 'dd/mm/yyyy')
                                            and w_a_data --to_date(w_a_data, 'dd/mm/yyyy')
        ;
cursor sel_tarsu_og(w_da_data date, w_a_data date, w_cod_fiscale varchar2) is
   select ogpr.titolo_occupazione titolo_occupazione
        , ogpr.natura_occupazione natura_occupazione
        , nvl(to_char(decode(to_char(ogva.dal, 'ddmmyyyy')
                            ,'0101'||to_char(to_number(to_char(p_da_data,'yyyy')) + 1), to_date('3112'||to_char(p_da_data,'yyyy'),'ddmmyyyy')
                            ,ogva.dal
                            )
                     , 'ddmmyyyy'), '01011900')
                                  inizio_occupazione
    ...
    where ogpr.oggetto                           = ogge.oggetto
      ...
      and ogva.oggetto_pratica      = nvl(ogpr.oggetto_pratica,ogpr.oggetto_pratica_rif)
...
begin
   ...
   w_anno_riferimento := to_char(p_da_data,'yyyy');
   --Record di testa
   w_numero := w_numero + 1;
   begin
   ...
   for rec_tarsu_sog in sel_tarsu_sog(p_da_data, p_a_data) loop
      for rec_tarsu_og in sel_tarsu_og(p_da_data, p_a_data, rec_tarsu_sog.cod_fiscale) loop
         --Record di dettaglio
         w_numero := w_numero + 1;
         begin
          insert into wrk_trasmissioni(numero
...
/* End Procedure: ESTRAZIONE_TARSU_AGENZIA */
/
