...
--changeset abrandolini:20250326_152423_calcolo_detrazioni_ici_ogge stripComments:false runOnChange:true 
 
create or replace procedure CALCOLO_DETRAZIONI_ICI_OGGE
/*******************************************************************************
  NOME:        CALCOLO_DETRAZIONI_ICI_OGGE
  DESCRIZIONE: Calcola le detrazioni ICI/IMU per contribuente/oggetto
  ANNOTAZIONI:
  REVISIONI:
...
--
   FOR rec_ogco IN sel_ogco (a_cf,a_anno,w_flag_pertinenze,a_ravvedimento,a_tipo_evento)
   LOOP
...
EXCEPTION
   ...
   WHEN OTHERS THEN
      ROLLBACK;
      RAISE_APPLICATION_ERROR(-20999,'Errore in Calcolo Detrazioni ICI Oggetto di '||w_cod_fiscale||' OGPR '||w_ogpr||' ('||SQLERRM||')');
END;
/* End Procedure: CALCOLO_DETRAZIONI_ICI_OGGE */
/
