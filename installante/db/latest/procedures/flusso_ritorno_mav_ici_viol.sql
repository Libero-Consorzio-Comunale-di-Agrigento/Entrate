...
--changeset abrandolini:20250326_152423_flusso_ritorno_mav_ici_viol stripComments:false runOnChange:true 
 
create or replace procedure FLUSSO_RITORNO_MAV_ICI_VIOL
   (
...
BEGIN
   ...
   FOR i IN 0 .. w_numero_righe - 1 LOOP
      ...
      if substr(w_riga, 2,2) = '14' and substr(w_riga, 98,7) = 'ICIVIOL' then
         ...
         w_ni14           := to_number(substr(w_codice_utente, 8,9));
      end if;
      if substr(w_riga, 2,2) = '51' then
         w_progressivo51  := to_number(substr(w_riga, 4,7));
         w_pratica        := to_number(substr(w_riga, 11,10));
         ...
         if w_progressivo51 = w_progressivo14 then
            ...
            if w_causale in ('07000','07011') and substr(w_codice_utente,1,7) = 'ICIVIOL' then
               begin
                  select anno
                    into w_anno
               ...
               EXCEPTION
                  WHEN others THEN
                     w_errore := ('Errore in recupero anno pratica:'|| to_char(w_pratica) || ' (' || SQLERRM || ')' );
                     raise errore;
               end;
               begin
                  select cod_fiscale
            ...
            elsif w_causale in ('07006','07008','07010') and substr(w_codice_utente,1,7) = 'ICIVIOL' then
...
END;
/* End Procedure: FLUSSO_RITORNO_MAV_ICI_VIOL */
/
