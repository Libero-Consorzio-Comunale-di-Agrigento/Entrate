...
--changeset abrandolini:20250326_152423_estrazione_liquidazioni_imu stripComments:false runOnChange:true 
 
CREATE OR REPLACE procedure     ESTRAZIONE_LIQUIDAZIONI_IMU
/*************************************************************************
 NOME:        ESTRAZIONE_LIQUIDAZIONI_IMU

 DESCRIZIONE: Estrazione per stampa avvisi di liquidazione IMU e relativi
              F24
              Comune di Pioltello
 ...
 004     21/09/2020   VD          Aggiunta gestione fabbricati merce
 003     03/01/2018   VD          Tolto simbolo "%" di fianco alle aliquote
 ...
 001     05/09/2017   EA          Aggiunte condizioni di where su
                                  numero e data notifica di pratiche_tributo
                                  come nel trattamento TARI
 000     27/07/2017   VD          Prima emissione.
...
is
  ...
  w_flag_int          number;
  w_imu_imp_dovuto    number;
  w_imu_imp_versato   number;
  w_imu_differenza    number;
...
begin
  ...
  for liq in ( select translate(sogg.cognome_nome, '/',' ') cognome_nome
                    ...
                    , decode(nvl(prtr.imposta_dovuta_totale,0)
                            ,0,rpad(' ', 27)
                            ,nvl(prtr.imposta_totale,0),rpad(' ', 27)
                            ,' SULLA BASE DELLA DENUNCIA ')    testo1
                    , decode(nvl(prtr.imposta_dovuta_totale,0)
                            ,0,rpad(' ', 62)
                            ,nvl(prtr.imposta_totale,0),rpad(' ', 62)
                            ,'TOTALE IMPOSTA CALCOLATA SULLA BASE DELLA RENDITA DEFINITIVA  ') testo2
                    , lpad(decode(nvl(prtr.imposta_dovuta_totale,0)
                                 ...
                                 ,translate(to_char(f_round(prtr.imposta_dovuta_totale,0),'99,999,999,990.00'), ',.', '.,')),21) imposta1
                    , lpad(decode(nvl(prtr.imposta_dovuta_totale,0)
                                 ,0,null
                                 ,nvl(prtr.imposta_totale,0),null
                                 ,translate(to_char(f_round(prtr.imposta_totale,0),'99,999,999,990.00'), ',.', '.,')),21) imposta2
  ...
  loop
    ...
    w_conta_righe_f24 := 0;
    w_num_campi       := 7;
    for dett in (select nvl(sanz.cod_tributo_f24,decode(sanz.cod_sanzione, 98,'3923'
                                                                         , 99,'3923'
                                                                         ,198,'3923'
                                                                         ,199,'3923'
                                                                             ,'3924')) cod_tributo
                      , round(sum(decode(nvl(sanz.cod_tributo_f24,decode(sanz.cod_sanzione, 98,'3923'
                                                                                          , 99,'3923'
                                                                                          ,198,'3923'
                                                                                          ,199,'3923'
                                                                                              ,'3924'))
               ...
               group by nvl(sanz.cod_tributo_f24,decode(sanz.cod_sanzione, 98,'3923'
                                                                         , 99,'3923'
                                                                         ,198,'3923'
                                                                         ,199,'3923'
                                                                             ,'3924'))
                 having round(sum(decode(nvl(sanz.cod_tributo_f24,decode(sanz.cod_sanzione, 98,'3923'
                                                                         , 99,'3923'
                                                                         ,198,'3923'
                                                                         ,199,'3923'
                                                                             ,'3924'))
    ...
    for ogg in ( select distinct
                       ...
                       ,translate(to_char(f_rendita(f_valore(ogpr_dic.valore
                         ...
                         rendita_valore_riv
                       ,decode(ogpr.valore
                              ,null, 'SULLA BASE DELLA RENDITA DEFINITIVA',null) st_pre_riog
                       ,decode(ogpr.valore
                              ,null, '[RIOG'
                              ,decode(ogpr.valore
                       ...
                       ,decode(f_cate_riog_null(ogpr.oggetto, prtr.anno)
                              ,null, null
                              ,rpad('CAT. CATASTALE SULLA BASE DELLA RENDITA DEFINITIVA', 54))
                         st_catren
                       ,decode(ogpr.valore
                              ,null, ''
                              ,decode(ogpr.valore
                       ...
                       ,decode(nvl(ogim.imposta_dovuta, 0)
                              ,nvl(ogim.imposta, 0), 'IMPOSTA CALCOLATA'||
                             decode(ogim.imposta_mini,null,'',' (MINI IMU 2013)')||decode(nvl(ogim.imposta_dovuta, 0),0,' : 0','')
                              ,'IMPOSTA CALCOLATA SULLA BASE DELLA DENUNCIA'||decode(ogim.imposta_mini,null,'',' (MINI IMU 2013)'))
                         imposta_testo1
                       ,decode(ogim.imposta_mini,null,''
                                                ,'(MINI IMU 2013)') st_mini_imu
                       ,decode(nvl(ogim.imposta_dovuta, 0),0,'','   Dettaglio codici tributo per versamento:') imposta_subtesto1
                       ,decode(nvl(ogim.imposta_dovuta, 0),0,to_char(''),translate(to_char(nvl(ogim.imposta_dovuta, 0)
                                                     ,'99,999,999,990.00')
                                             ,',.'
                                             ,'.,')
                                   )          importo1
                       ...
                       ,decode(nvl(ogim.imposta, 0), nvl(ogim.imposta_dovuta, 0), null
                              , 'IMPOSTA CALCOLATA SULLA BASE DELLA RENDITA DEFINITIVA'||decode(nvl(ogim.imposta_dovuta, 0),0,' : 0',''))
                         imposta_testo2
                       ,decode(nvl(ogim.imposta, 0), nvl(ogim.imposta_dovuta, 0), null
    ...
    loop
      ...
      for cotr in (select translate (to_char (sum (decode (ogim.tipo_aliquota,
                   ...
                   select translate
                     ...
                     from oggetti_imposta ogim, oggetti_pratica ogpr, oggetti ogge, aliquote aliq
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      and ogpr.oggetto = ogge.oggetto
                      and ogpr.pratica = liq.pratica
                      and ogpr.oggetto_pratica = ogg.oggetto_pratica
                   ...
                   having sum (nvl (ogim.imposta_dovuta, 0)) != 0
                      and sum (decode (ogim.aliquota_erariale, null, nvl (ogim.imposta_dovuta, 0), 0)) != 0
                    union
                   select translate
                             (to_char (sum (decode (nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto),
                                                    1, nvl (ogim.imposta_dovuta, 0)
                              ...
                              ',.',
                              '.,'
                             ) imposta_ter_com_3914,
                          '3914' codice_tributo, rpad ('IMU - TERRENI - COMUNE', 44) des_tributo
                     from oggetti_imposta ogim, oggetti_pratica ogpr, oggetti ogge
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      and ogpr.oggetto = ogge.oggetto
                      and ogpr.pratica = liq.pratica
                   ...
                   having sum (nvl (ogim.imposta_dovuta, 0) - nvl (ogim.imposta_erariale_dovuta, 0)) != 0
                    union
                   select translate
                             (to_char (sum (decode (nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto),
                                                    1, nvl (ogim.imposta_erariale_dovuta, 0),
                                                    0
                                                   )
                                           ),
                                       '99,999,999,990.00'
                   ...
                   select translate
                             (to_char (sum (decode (nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto),
                                                    2, nvl (ogim.imposta_erariale_dovuta, 0),
                                                    0
                                                   )
                                           ),
                                       '99,999,999,990.00'
                   ...
                   select translate
                    ...
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      ...
                      and nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto) not in (1, 2)
                      and aliquota_erariale is not null
                      and f_altri_importo (liq.pratica, ogg.oggetto_pratica, 'STATO', ogpr.anno, 'DOVUTA') > 0
                   union
                   select translate
                             (to_char
                                               ...
                                               9, decode
                                                        (nvl (ogpr.tipo_oggetto,
                                                         ...
                                                         decode (substr (ogpr.categoria_catasto,
                                                                 ...
                                                                 'D', nvl (ogim.imposta_dovuta,
                                                                           0)
                                                                  - nvl (ogim.imposta_erariale_dovuta,
                                                                         0),
                                                                 0
                                                                )
                                                        ),
                    ...
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      ...
                      and ogpr.pratica = liq.pratica
                      and ogpr.oggetto_pratica = ogg.oggetto_pratica
                      and ogim.tipo_aliquota = 9
                      and nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto) not in (1, 2)
                      and substr (ogpr.categoria_catasto, 1, 1) = 'd'
                      and ogpr.anno >= 2013
                   having sum (nvl (ogim.imposta_dovuta, 0) - nvl (ogim.imposta_erariale_dovuta, 0)) != 0
                   union
                   select translate
                                               ...
                                               9, decode
                                                        (nvl (ogpr.tipo_oggetto,
                                                         ...
                                                         decode (substr (ogpr.categoria_catasto,
                                                                 ...
                                                                 'D', nvl (ogim.imposta_erariale_dovuta,
                                                                           0
                                                                          ),
                                                                 0
                                                                )
                   ...
                   select translate
                          ...
                          '3939' codice_tributo,
                          rpad ('IMU - IMM.COSTR. E DEST.ALLA VEND. - COMUNE', 44) des_tributo
                     from oggetti_imposta ogim, oggetti_pratica ogpr, oggetti ogge, aliquote aliq
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      and ogpr.oggetto = ogge.oggetto
                      and ogpr.pratica = liq.pratica
      ...
      for cotr in (select translate (to_char (sum (decode (ogim.tipo_aliquota,
                    ...
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      ...
                      and ogpr.oggetto_pratica = ogg.oggetto_pratica
                      and ogim.tipo_aliquota = 2
                      and nvl (ogim.imposta_dovuta, 0) != nvl (ogim.imposta, 0)
                   ...
                   select translate
                    ...
                    where ogpr.oggetto_pratica = ogim.oggetto_pratica
                      ...
                      and nvl (ogim.imposta_dovuta, 0) != nvl (ogim.imposta, 0)
                   having sum (nvl (ogim.imposta, 0)) != 0
                      and sum (decode (ogim.aliquota_erariale, NULL, nvl (ogim.imposta, 0), 0)) != 0
                   union
                  select translate
                            (to_char (sum (decode (nvl (ogpr.tipo_oggetto, ogge.tipo_oggetto),
                 ...
                 select translate
                        ...
                        '3915' codice_tributo, rpad ('IMU - TERRENI - STATO', 44) des_tributo
                   from oggetti_imposta ogim, oggetti_pratica ogpr, oggetti ogge
                  where ogpr.oggetto_pratica = ogim.oggetto_pratica
                    and ogpr.oggetto = ogge.oggetto
                    and ogpr.pratica = liq.pratica
                ...
                select translate
                 ...
                 where ogpr.oggetto_pratica = ogim.oggetto_pratica
                   ...
                   and aliquota_erariale IS NOT NULL
                   and nvl (ogim.imposta_dovuta, 0) != nvl (ogim.imposta, 0)
                   and f_altri_importo (liq.pratica, ogg.oggetto_pratica, 'STATO', ogpr.anno, 'RENDITA') > 0
                union
               select translate
                         (to_char
                             (sum (decode (ogim.tipo_aliquota,
               ...
               select translate
                      ...
                      '3939' codice_tributo,
                      rpad ('IMU - IMM. COSTRUITI E DESTINATI ALLA VENDITA - COMUNE', 44) des_tributo
                 from oggetti_imposta ogim, oggetti_pratica ogpr, oggetti ogge, aliquote aliq
                where ogpr.oggetto_pratica = ogim.oggetto_pratica
                  and ogpr.oggetto = ogge.oggetto
                  and ogpr.pratica = liq.pratica
    ...
    --
    w_ind := w_ind + 1;
    w_riga := 'IMPOSTA COMPLESSIVA';
    riga_file (w_ind) := lpad(' ',trunc((99 - length(w_riga)) / 2))||w_riga;
    w_ind := w_ind + 1;
    riga_file (w_ind) := 'TOTALE IMPOSTA CALCOLATA'||liq.testo1||'                         '||liq.imposta1;
    w_ind := w_ind + 1;
    riga_file (w_ind) := liq.testo2||'              '||liq.imposta2;
    --
    ...
    for vers in (select vers.importo_versato
                      ...
                      , decode(vers.tipo_versamento,'A','ACCONTO '
                                                   ...
                                                   ,'U','UNICO   '
                                                       ,'      ') tipo_versamento
                       ,decode(f_get_tipo_imu(liq.pratica)
                              ,2,to_char(f_scadenza_mini_imu(vers.anno,liq.pratica),'dd/mm/yyyy')
                       ...
                       ,decode(f_get_tipo_imu(liq.pratica)
                              ,2,decode(sign(trunc(f_scadenza_mini_imu(vers.anno,liq.pratica) - vers.data_pagamento))
                                       ,-1,lpad(trunc(f_scadenza_mini_imu(vers.anno,liq.pratica) - vers.data_pagamento),6)
                                 ,'')
                                ,decode(sign(trunc(f_scadenza(vers.anno
                                      ...
                                      ,'')) gio_dif
                         ,decode(f_get_tipo_imu(liq.pratica),0
                                ,rpad(' ',30)||'Scadenza Mini IMU      ' ||
                                 to_char(f_scadenza_mini_imu(vers.anno,liq.pratica),'dd/mm/yyyy')||lpad(' ',10)||
                                 decode(sign(trunc(f_scadenza_mini_imu(vers.anno,liq.pratica) - vers.data_pagamento))
                                       ,-1,lpad(trunc(f_scadenza_mini_imu(vers.anno,liq.pratica) - vers.data_pagamento),6)
                                ,'')
                            ,'') dati_mini_imu
                       ,1 ord
    ...
    loop
      ...
      riga_file (w_ind) :=  vers.tipo_versamento||lpad(' ',3)||
                            ...
                            vers.gio_dif||'  '||vers.dati_mini_imu;
      --
    ...
    --
    -- Importi per comune (divisi per codice tributo)
    --
    for cotr in (select rpad('IMU - AB. PRINC. E PERT.', 32) descrizione
                      , 3912 codice
                      , sum(decode(ogim.tipo_aliquota,2,ogim.imposta
                                                       ,0))
                         dovuto
                 ...
                 having sum(decode(ogim.tipo_aliquota, 2, ogim.imposta, 0)) > 0
                     or   max(vers.ab_principale)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - FABB. RUR. AD USO STRUM.', 32)
                      , 3913
                      , sum(decode(ogim.tipo_aliquota
                                  ,2,0
                                    ,decode(nvl(ogpr.tipo_oggetto
                 ...
                 having sum(decode(ogim.tipo_aliquota
                                  ...
                                  ,decode(nvl(ogpr.tipo_oggetto, ogge.tipo_oggetto)
                                         ...
                                         ,decode(aliq.flag_fabbricati_merce
                                                ,'S', 0
                                                ,decode(ogim.aliquota_erariale, null, ogim.imposta, 0))))) >
                         0
                     or   max(vers.rurali)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ,w_tipo_tributo
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - TERRENI', 32)
                      , 3914
                      , sum(decode(nvl(ogpr.tipo_oggetto,ogge.tipo_oggetto)
                                  ,1,ogim.imposta - nvl(ogim.imposta_erariale, 0)
                                    ,0))
                      , max(vers.terreni_comune) + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                                               ,w_tipo_tributo
                                                                               ,liq.anno
                                                                               ,'U'
                                                                               ,'TEC'
                 ...
                 having sum(decode(nvl(ogpr.tipo_oggetto, ogge.tipo_oggetto)
                     ...
                     or   max(vers.terreni_comune)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - AREE FABBRICABILI', 32)
                      , 3916
                      , sum(decode(nvl(ogpr.tipo_oggetto,ogge.tipo_oggetto)
                 ...
                 having sum(decode(nvl(ogpr.tipo_oggetto, ogge.tipo_oggetto)
                     ...
                     or   max(vers.aree_comune)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - ALTRI FABBRICATI', 32)
                      , 3918
                 ...
                 having sum(decode(ogim.tipo_aliquota
                     ...
                     or   max(vers.altri_comune)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - IMM. PROD. (GR.C.D)-INC.', 32)
                      , 3930
                      , sum(decode(ogim.tipo_aliquota
                                  ,2, 0
                                  ,decode(nvl(ogpr.tipo_oggetto
                                         ...
                                         ,decode(aliquota_erariale
                                                ...
                                                ,decode(sign(  ogim.anno
                                                       ...
                                                       ,1, decode(   substr(ogpr.categoria_catasto
                                                                 ...
                                                                 ,0)
                                                       ,0)))))
                      , max(vers.fabb_d_comune) + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                                              ,w_tipo_tributo
                                                                              ,liq.anno
                                                                              ,'U'
                                                                              ,'FDC'
                 ...
                 having sum(decode(ogim.tipo_aliquota
                     ...
                     or   max(vers.fabb_d_comune)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - FABB. DEST. VENDITA', 32) descrizione
                      , 3939 codice
                      , sum(decode(aliq.flag_fabbricati_merce,'S',ogim.imposta
                                                             ,0))
                         dovuto
                      , max(vers.fabbricati_merce) + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                                                 ,w_tipo_tributo
                                                                                 ,liq.anno
                                                                                 ,'U'
                                                                                 ,'FAM'
    ...
    loop
      ...
      --
      w_imu_imp_dovuto  := w_imu_imp_dovuto + cotr.dovuto;
      w_imu_imp_versato := w_imu_imp_versato + cotr.versato;
      w_imu_differenza  := w_imu_differenza + cotr.differenza;
      --
      w_ind := w_ind + 1;
      riga_file(w_ind) := w_riga||cotr.descrizione||cotr.codice||
    ...
    --
    -- Totale importi per comune
    --
    w_ind := w_ind + 1;
    riga_file (w_ind) := lpad(' ', 43)||rpad(' _', 18, '_')||rpad(' _', 18, '_')||rpad(' _', 18, '_');
    w_ind := w_ind + 1;
    riga_file (w_ind) := lpad('Comune Totale',43)||
                         lpad(translate(to_char(w_imu_imp_dovuto, '99,999,999,990.00'),',.','.,'),18)||
                         lpad(translate(to_char(w_imu_imp_versato,'99,999,999,990.00'),',.','.,'),18)||
                         lpad(translate(to_char(w_imu_differenza,'99,999,999,990.00'),',.','.,'),18);
    w_ind := w_ind + 1;
    riga_file (w_ind) := ' ';
    --
    w_tot_imp_dovuto  := w_tot_imp_dovuto + w_imu_imp_dovuto;
    w_tot_imp_versato := w_tot_imp_versato + w_imu_imp_versato;
    w_tot_differenza  := w_tot_differenza + w_imu_differenza;
    ...
    w_imu_differenza  := 0;
    --
    -- Importi per stato (divisi per codice tributo)
    --
    for cotr in (select rpad('IMU - TERRENI', 32) descrizione
                      , 3915 codice
                      , sum(decode(nvl(ogpr.tipo_oggetto,ogge.tipo_oggetto)
                                  ,1,imposta_erariale
                                    ,0)) dovuto
                 ...
                 having sum(decode(ogim.tipo_aliquota
                     ...
                     or   max(vers.altri_erariale)
                        + max(f_importo_vers_ravv_dett(liq.cod_fiscale
                                                      ...
                                                      ,liq.data_liq)) > 0
                 union
                 select rpad('IMU - IMM. PROD. (GR.C.D)', 32)
                      , 3925
                      , sum(decode(ogim.tipo_aliquota,2,0
                                  ,decode(nvl(ogpr.tipo_oggetto, ogge.tipo_oggetto)
                                         ,1, 0
    ...
    --
    -- Totale importi per erario
    --
    if w_imu_imp_dovuto <> 0 or
       w_imu_imp_versato <> 0 or
       w_imu_differenza <> 0 then
       w_ind := w_ind + 1;
       riga_file (w_ind) := lpad(' ', 43)||rpad(' _', 18, '_')||rpad(' _', 18, '_')||rpad(' _', 18, '_');
       w_ind := w_ind + 1;
       riga_file (w_ind) := lpad('Stato Totale',43)||
                            lpad(translate(to_char(w_imu_imp_dovuto, '99,999,999,990.00'),',.','.,'),18)||
                            lpad(translate(to_char(w_imu_imp_versato,'99,999,999,990.00'),',.','.,'),18)||
                            lpad(translate(to_char(w_imu_differenza,'99,999,999,990.00'),',.','.,'),18);
       --
       ...
       riga_file (w_ind) := lpad(' ', 43)||rpad(' _', 18, '_')||rpad(' _', 18, '_')||rpad(' _', 18, '_');
       w_ind := w_ind + 1;
       riga_file (w_ind) := lpad('Totale Complessivo',43)||
                            lpad(translate(to_char(w_tot_imp_dovuto, '99,999,999,990.00'),',.','.,'),18)||
                            lpad(translate(to_char(w_tot_imp_versato,'99,999,999,990.00'),',.','.,'),18)||
                            lpad(translate(to_char(w_tot_differenza,'99,999,999,990.00'),',.','.,'),18);
    end if;
    --
    ...
    for sanz in ( select sapr.cod_sanzione
                    ...
                    where sapr.cod_sanzione = sanz.cod_sanzione (+) and
                          sapr.sequenza_sanz = sanz.sequenza (+) and
...
/* End Procedure: ESTRAZIONE_LIQUIDAZIONI_IMU */
/
